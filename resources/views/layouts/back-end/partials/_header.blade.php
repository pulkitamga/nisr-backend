...
<style>
    #notificationDropdown a.dropdown-item span:last-child {
        font-size: 11px;
    }

    #notificationDropdown a.dropdown-item span:first-child {
        font-size: 12px;
    }

    #notificationDropdown .dropdown-item:not(:last-child) {
        border-bottom: 0.5px solid #073b74;
    }
        ...
        <div class="navbar-nav-wrap">
            ...
            <div class="navbar-nav-wrap-content-right"
                ...
                <ul class="navbar-nav align-items-center flex-row gap-xl-16px">
                    ...
                    <li class="nav-item">
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                href="{{route('admin.crm.index')}}" title="{{translate('message')}}" data-toggle="tooltip" data-custom-class="header-icon-title">
                                <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                @php($message=\App\Models\InboxMessage::where('status','new')->count())
                                @if($message!=0)
                    ...
                    // Check if this admin is a department head
                    $department = Departments::where('head_id', $authAdminId)->first();

                    if ($department) {
                        $notifications = Notifications::getDepartmentNotifications($authAdminId, [0]);
                    } else {
                        $notifications = Notifications::getNotifications($authAdminId, [0]);
                    };
                    ?>
                    <li class="nav-item {{ auth('admin')->id() == 1 ? 'd-none' :'' }}">
                        <div class="hs-unfold">
                            ...
                            <div id="notificationDropdown" class="hs-unfold-content dropdown-unfold dropdown-menu dropdown-menu-right navbar-dropdown-menu py-2" style="border: 1px solid #ccc;">
                                ...
                                <a href="{{ route('admin.notifications.view', $notification->id) }}"
                                    ...
                                    <span>
                                        {{-- Try to show a human readable source, fallback to title --}}
                                        ...
                                        @else
                                        {{ Str::limit($notification->title, 38, '...') }}
                                        @endif
                                    </span><br>
                                    <span style="font-size: 12px; color:#6e717a">
                                        {{ Str::limit($notification->message, 40, '...') }}
                                    </span><br>
                                    <span class="text-muted text-sm"><i class="tio-time"></i>
                                        @if($notification->created_at)
                                        {{ Carbon::parse($notification->created_at)->diffInDays(now()) < 7
                                            ? Carbon::parse($notification->created_at)->format('D h:i A')
                                            : Carbon::parse($notification->created_at)->format('d M Y h:i A') }}
                                        @else
                                        N/A
                                        @endif
                                    </span>
                                ...
                                @empty
                                <a href="#" class="dropdown-item px-2 py-1 text-center text-muted">
