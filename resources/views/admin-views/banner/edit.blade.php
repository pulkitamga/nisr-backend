                        ...
                        <form action="{{ route('admin.banner.update', [$banner['id']]) }}" method="post" enctype="multipart/form-data"
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    @endphp
                                    <ul class="nav nav-tabs w-fit-content mb-4 input-field-for-main-banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}}">
                                    ...
                                    @endforeach
                                    @foreach($language as $lang)
                                    <div class="form-group mt-4 input-field-for-main-banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}} {{ $lang != $activeLanguage ? 'd-none' : '' }} form-system-language-form"
                                         id="{{$lang}}-form">
                                        ...
                                        <input type="text" name="button_text[]" class="form-control" id="button_text_{{$lang}}" placeholder="{{ translate('Enter_button_text') }}"
                                               value="{{$lang == $defaultLanguage ? $banner->getRawOriginal('button_text') : $banner->getTranslatedField('button_text', $lang, '') }}">
                                    </div>
                                    @endforeach
                                        ...
                                        <p>{{ translate('banner_Image_ratio_is_not_same_for_all_sections_in_website') }}. {{ translate('please_review_the_ratio_before_upload') }}</p>
                                         ...
                                         <div class="form-group mt-4 input-field-for-main-banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}} {{ $lang != $activeLanguage ? 'd-none' : '' }} form-system-language-form"
                                             ...
                                             <input type="text" name="title[]" class="form-control" id="title_{{$lang}}" placeholder="{{ translate('Enter_banner_title') }}"
                                                    value="{{$lang == $defaultLanguage ? $banner->getRawOriginal('title') : $banner->getTranslatedField('title', $lang, '') }}">
                                         </div>
                                         @endforeach
                                         @foreach($language as $lang)
                                         <div class="form-group mb-0 input-field-for-main-banner {{$banner['banner_type'] !='Main Banner'?'d-none':''}} {{ $lang != $activeLanguage ? 'd-none' : '' }} form-system-language-form"
                                             ...
                                             <input type="text" name="sub_title[]" class="form-control" id="sub_title_{{$lang}}" placeholder="{{ translate('Enter_banner_sub_title') }}"
                                                    value="{{$lang == $defaultLanguage ? $banner->getRawOriginal('sub_title') : $banner->getTranslatedField('sub_title', $lang, '') }}">
                                         </div>
                                         @endforeach
                                         @endif
                                    </div>
