    ...
    @foreach ($products as $key => $product)
        <div class="select-product-item media gap-3 border-bottom py-2 cursor-pointer action-select-search-product"
            ...
            <div class="media-body d-flex flex-column gap-1">
                ...
                <div
                    class="fz-10">{{translate('Category').' '.':'.' '}}{{isset($product->category) ? $product->category->getTranslatedField('name') : translate('category_not_found') }}</div>
                <div
                    class="fz-10">{{translate('Brand').' '.':'.' '}}{{isset($product->brand) ? $product->brand->getTranslatedField('name') : translate('brands_not_found') }}</div>
