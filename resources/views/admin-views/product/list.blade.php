...
<div class="content container-fluid">
                        ...
                        <div class="form-group">
                            ...
                            <select class="js-select2-custom form-control" name="sub_sub_category_id"
                                ...
                                <option
                                    ...
                                    {{request('sub_sub_category_id') != null ? $subSubCategory['defaultName'] :
                ...
                <div class="px-3 py-4">
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                            ...
                            @if($type == 'in_house')
                            <a href="{{ route('admin.products.stock-limit-list',['in_house']) }}" class="btn btn-info">
    ...
    Str::limit(
        $product->product_type === 'physical'
            ? ($product->name ?? '')
            : $product->getServiceTitle(getDefaultLanguage()),
        20
    ) 
}}
                                            </div>
                                ...
                                <td class="text-center">
                                    @if($product['product_type'] == 'services' && isset($product->service))
                                    {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $product->service->base_price_inshop), currencyCode: getCurrencyCode()) }}
                                    @else
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('admin.products.product-featured-status') }}" method="post"
                                        id="product-featured{{ $product['id']}}-form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $product['id']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input toggle-switch-message"
                                                name="status" id="product-featured{{ $product['id'] }}" value="1" {{
                                ...
                                <td class="text-center">
                                    ...
                                    @php($productName = str_replace("'",'`',$product['name']))
                                    <form action="{{ route('admin.products.product-cms-status') }}" method="post"
                                        id="product-cms-status{{ $product['id']}}-form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $product['id']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input toggle-switch-message"
                                                name="status" id="product-cms-status{{ $product['id'] }}" value="1" {{
                                                $product['show_cms']==1 ? 'checked' : '' }}
                                                data-modal-id="toggle-status-modal"
                                                data-toggle-id="product-cms-status{{ $product['id'] }}"
                                                data-on-image="product-status-on.png"
                                                data-off-image="product-status-off.png"
                                                data-on-title="{{ translate('Want_to_Show').' '.$productName.' '.translate('to_the_home_page') }}"
                                                data-off-title="{{ translate('Want_to_Hide').' '.$productName.' '.translate('from_the_featured_section') }}"
                                                data-on-message="<p>{{ translate('if_enabled_this_product_will_be_shown_in_the_home_page_on_the_website') }}</p>"
                                                data-off-message="<p>{{ translate('if_disabled_this_product_will_be_removed_from_the_home_page_on_the_website') }}</p>">
                                            <span class="switcher_control"></span>
                                        </label>
                                    </form>
                                ...
                                <td class="text-center">
                                    ...
                                    @php($productName = str_replace("'",'`',$product['name']))
                                    <form action="{{ route('admin.products.product-showcase-status') }}" method="post"
                                        id="product-showcase-status{{ $product['id']}}-form">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $product['id']}}">
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input toggle-switch-message"
                                                name="status" id="product-showcase-status{{ $product['id'] }}" value="1" {{
                                                $product['showcase_product']==1 ? 'checked' : '' }}
                                                data-modal-id="toggle-status-modal"
                                                data-toggle-id="product-showcase-status{{ $product['id'] }}"
                                                data-on-image="product-status-on.png"
                                                data-off-image="product-status-off.png"
                                                data-on-title="{{ translate('Want_to_Show').' '.$productName.' '.translate('to_the_product_page') }}"
                                                data-off-title="{{ translate('Want_to_Hide').' '.$productName.' '.translate('from_the_product_section') }}"
                                                data-on-message="<p>{{ translate('if_enabled_this_product_will_be_shown_in_the_product_page_on_the_website') }}</p>"
                                                data-off-message="<p>{{ translate('if_disabled_this_product_will_be_removed_from_the_product_page_on_the_website') }}</p>">
                                            <span class="switcher_control"></span>
                                        </label>
                                    </form>
                                ...
                                <td class="text-center">
                                    <form action="{{ route('admin.products.status-update') }}" method="post"
                                        ...
                                        <label class="switcher mx-auto">
                                            <input type="checkbox" class="switcher_input toggle-switch-message"
                                                name="status" id="product-status{{ $product['id'] }}" value="1" {{
