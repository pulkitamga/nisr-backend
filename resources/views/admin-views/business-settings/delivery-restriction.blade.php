...
<div class="content container-fluid">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto min-form-control-height mt-2" id="customer_wallet_section">
                        <span class="title-color">
                            {{translate('delivery_available_state')}}
                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="right" title="{{translate('if_enabled,_you_can_choose_one_or_multiple_states_for_product_delivery').'.' }}">
                                <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                            </span>
                        </span>
                        <form action="{{ route('admin.business-settings.delivery-restriction.state-restriction-status-change') }}" method="post" id="state-area-form" data-from="delivery-restriction">
                            ...
                            <label class="switcher">
                                <input type="checkbox" class="switcher_input toggle-switch-message" name="status" id="state-area" data-level="state" {{ isset($stateRestrictionStatus->value) && $stateRestrictionStatus->value  == 1 ? 'checked' : '' }} value="1"
                                    ...
                                    data-on-image="delivery-available-country-on.png"
                                    data-off-image="delivery-available-country-off.png"
                                    data-on-title="{{translate('want_to_Turn_ON_Delivery_Available_State').'?'}}"
                                    data-off-title="{{translate('want_to_Turn_OFF_Delivery_Available_State').'?'}}"
                                    data-on-message="<p>{{translate('if_enabled_the_admin_or_vendor_can_deliver_orders_to_the_selected_states')}}</p>"
                                    data-off-message="<p>{{translate('if_disabled_there_will_be_no_delivery_restrictions_for_admin')}}</p>">
                                <span class="switcher_control"></span>
                            </label>
                        </form>
                ...
                <div class="col-md-4">
                    <div class="d-flex justify-content-between align-items-center gap-10 form-control h-auto min-form-control-height mt-2" id="customer_wallet_section">
                        <span class="title-color">
                            {{translate('delivery_available_city')}}
                            <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="right" title="{{translate('if_enabled,_you_can_choose_one_or_multiple_cities_for_product_delivery').'.' }}">
                                <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                            </span>
                        </span>
                        <form action="{{ route('admin.business-settings.delivery-restriction.city-restriction-status-change') }}" method="post" id="city-area-form" data-from="delivery-restriction">
                            ...
                            <label class="switcher">
                                <input type="checkbox" class="switcher_input toggle-switch-message" name="status" id="city-area" data-level="city" {{ isset($cityRestrictionStatus->value) && $cityRestrictionStatus->value  == 1 ? 'checked' : '' }} value="1"
                                    ...
                                    data-on-image="delivery-available-country-on.png"
                                    data-off-image="delivery-available-country-off.png"
                                    data-on-title="{{translate('want_to_Turn_ON_Delivery_Available_City').'?'}}"
    ...
    <div class="row gy-2">
        <div class="col-lg-4 {{ isset($countryRestrictionStatus->value) && $countryRestrictionStatus->value  != 1 ? 'd-none' : '' }}">
            <div class="card mb-3">
        ...
        <div class="col-lg-4 {{ isset($stateRestrictionStatus->value) && $stateRestrictionStatus->value  != 1 ? 'd-none' : '' }}">
            <div class="card mb-3">
                <div class="card-body country-disable">
                    <form action="{{ route('admin.business-settings.delivery-restriction.add-delivery-state') }}"
                        method="post">
                        @csrf
                        <div class="form-group">
                            <label class="title-color d-flex font-weight-bold">{{translate('State')}} </label>
                            <div class="d-flex gap-2">
                                <select class="js-example-basic-multiple js-states js-example-responsive form-control"
                                    name="state[]" id="choice_attributes_state" multiple="multiple">
                                    @foreach($states as $state)
                                    <option value="{{ $state['id'] }}" {{ in_array($state['id'], $storedStatesIds) ? 'disabled' : '' }}>
                                        {{ $state['name'] }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mt-6">
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tbody>
                                    @foreach($storedStates as $key => $store)
                                    <tr>
                                        <td class="">{{ $storedCountries->firstItem() + $key }}</td>
                                        <td class="text-center">{{ $store->state->name ?? '' }}</td>
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-danger btn-sm square-btn delete-data"
                                                ...
                                                <form
                                                    action="{{route('admin.business-settings.delivery-restriction.delivery-state-delete',['id' => $store->id])}}"
                                                    method="post" id="state-{{$store->id}}">
                                                    @csrf @method('delete')
                                                </form>
                                            </div>
                                        </td>
        ...
        <div class="col-lg-4 {{ isset($cityRestrictionStatus->value) && $cityRestrictionStatus->value  != 1 ? 'd-none' : '' }}">
            <div class="card mb-3">
                <div class="card-body city-disable">
                    <form action="{{ route('admin.business-settings.delivery-restriction.add-delivery-city') }}"
                        method="post">
                        @csrf
                        <div class="form-group">
                            <label class="title-color d-flex font-weight-bold">{{translate('City')}} </label>
                            <div class="d-flex gap-2">
                                <select class="js-example-basic-multiple js-city js-example-responsive form-control"
                                    name="city[]" id="choice_attributes_city" multiple="multiple">
                                    @foreach($cities as $city)
                                    <option value="{{ $city['id'] }}" {{ in_array($city['id'], $storedCitiesIds) ? 'disabled' : '' }}>
                                        {{ $city['name'] }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mt-6">
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tbody>
                                    @foreach($storedCities as $key => $store)
                                    <tr>
                                        <td class="">{{ $storedCities->firstItem() + $key }}</td>
                                        <td class="text-center">{{ $store->city->name ?? '' }}</td>
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-danger btn-sm square-btn delete-data"
                                                ...
                                                <form
                                                    action="{{route('admin.business-settings.delivery-restriction.delivery-city-delete',['id' => $store->id])}}"
                                                    method="post" id="city-{{$store->id}}">
                                                    @csrf @method('delete')
                                                </form>
                                            </div>
                                        </td>
        ...
        <div class="col-lg-4  {{ isset($zipCodeAreaRestrictionStatus) && $zipCodeAreaRestrictionStatus->value  != 1? 'd-none' : '' }}">
            <div class="card mb-3">
        ...
        <div class="col-lg-4  {{ isset($areaRestrictionStatus) && $areaRestrictionStatus->value  != 1? 'd-none' : '' }}">
            <div class="card mb-3">
                <div class="card-body area-disable">
                    <form action="{{ route('admin.business-settings.delivery-restriction.add-area') }}"
                        method="post">
                        @csrf
                        <div class="form-group">
                            <label class="title-color d-flex font-weight-bold">{{translate('Area')}} </label>
                            <div class="d-flex gap-2">
                                <select class="js-example-basic-multiple js-area js-example-responsive form-control"
                                    name="area[]" id="choice_attributes_area" multiple="multiple">
                                    @foreach($areas as $area)
                                    <option value="{{ $area['id'] }}" {{ in_array($area['id'], $storedAreasIds) ? 'disabled' : '' }}>
                                        {{ $area['name'] }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="mt-6">
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tbody>
                                    @foreach($storedArea as $key => $area)
                                    <tr>
                                        <td>{{ $storedArea->firstItem() + $key }}</td>
                                        <td class="text-center">{{ optional($area->areaInfo)->name ?? translate('area_not_found') }}
                                        </td>
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                <a class="btn btn-outline-danger btn-sm square-btn delete-data" href="javascript:"
                                                ...
                                                <form
                                                    action="{{route('admin.business-settings.delivery-restriction.area-delete',['id' => $area->id])}}"
