...
@section('title', translate('quotation_Settings'))
@section('content')
<div class="content container-fluid">
    ...
    <div class="card">
        ...
        <div class="card-header">
            ...
            <div class="form-group d-flex align-items-center gap-3">
                <label class="switcher mx-auto">
                    <input type="checkbox" class="switcher_input status-toggle" data-type="quotation_settings"
        ...
        <div class="card-body">
            <form action="{{route('admin.business-settings.quotation-settings.update')}}" method="post" enctype="multipart/form-data" id="update-invoice-settings">
                @csrf
                <div class="row g-3">
                    <div class="col-md-12 d-flex flex-column justify-content-center mb-4">
                        <div>
                            <label class="title-color text-capitalize">
                                {{ translate('header_logo') }}
                            ...
                            <div class="mx-auto text-center">
                                <div class="uploadDnD">
                                    @php
                                    $imageArray = [
                                    'image_name' => $invoiceSettings?->image_header->image_name ?? $invoiceSettings?->image,
                                    'storage' => $invoiceSettings?->image_header?->storage ?? 'public',
                                    ];
                                    $imagePath = storageLink('company',$imageArray['image_name'],$imageArray['storage']);
                                    @endphp
                                    <div class="form-group inputDnD input_image_header input_image_edit"
                                        data-bg-img="{{$imagePath['path']}}"
                                        data-title="{{ !is_null($imagePath['path']) ? '': translate('drag_and_drop_file_or_Browse_file')}}">
                                        <input type="file" name="image_header" class="form-control-file text--primary font-weight-bold" id="quotation-image-header" accept=".jpg, .png, .jpeg, .gif, .bmp, .webp |image/*">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mx-auto text-center">
                                <div class="uploadDnD">
                                    @php
                                    $imageArray = [
                                    'image_name' => $invoiceSettings?->image_bg->image_name ?? $invoiceSettings?->image,
                                    'storage' => $invoiceSettings?->image_bg?->storage ?? 'public',
                                    ];
                                    $imagePath = storageLink('company',$imageArray['image_name'],$imageArray['storage']);
                                    @endphp
                                    <div class="form-group inputDnD input_image_bg input_image_edit"
                                        data-bg-img="{{$imagePath['path']}}"
                                        data-title="{{ !is_null($imagePath['path']) ? '': translate('drag_and_drop_file_or_Browse_file')}}">
                                        <input type="file" name="image_bg" class="form-control-file text--primary font-weight-bold" id="quotation-image-bg" accept=".jpg, .png, .jpeg, .gif, .bmp, .webp |image/*">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mx-auto text-center">
                                <div class="uploadDnD">
                                    @php
                                    $imageArray = [
                                    'image_name' => $invoiceSettings?->image_footer->image_name ?? $invoiceSettings?->image,
                                    'storage' => $invoiceSettings?->image_footer?->storage ?? 'public',
                                    ];
                                    $imagePath = storageLink('company',$imageArray['image_name'],$imageArray['storage']);
                                    @endphp
                                    <div class="form-group inputDnD input_image_footer input_image_edit"
                                        data-bg-img="{{$imagePath['path']}}"
                                        data-title="{{ !is_null($imagePath['path']) ? '': translate('drag_and_drop_file_or_Browse_file')}}">
                                        <input type="file" name="image_footer" class="form-control-file text--primary font-weight-bold" id="quotation-image-footer" accept=".jpg, .png, .jpeg, .gif, .bmp, .webp |image/*">
                                    </div>
                                </div>
                            </div>
                        </div>
