        ...
        <div class="card-body">
            <form action="{{route('admin.business-settings.order-settings.update-order-settings')}}" method="post"
                ...
                <div class="row align-items-end">
                    ...
                    @php($businessMode = $web_config['business_mode'] ?? 'single')
                    @if($businessMode !== 'multi' )
                    <input type="hidden" name="free_delivery_responsibility" value="admin">
                    @else
                    <div class="col-xl-4 col-md-6">
                        <div class="form-group">
                            <label class="title-color d-flex text-capitalize" for="free_delivery_responsibility">
                    ...
                    @php($stockVerify=getWebConfig('stock_check'))
                    <div class="col-xl-4 col-md-6">
                        <div
                            class="d-flex justify-content-between align-items-center gap-10 form-control form-group">
                            <span class="title-color d-flex align-items-center gap-1">
                                {{translate('stock_Check')}}
                                <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="top"
                                    title="{{translate('if_enabled_users_can_complete_the_checkout_process_without_checking_stock_is_availble_or_not')}}">
                                    <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                                </span>
                            </span>
                            <label class="switcher" for="stock_check">
                                <input type="checkbox" value="1" class="switcher_input toggle-switch-message" name="stock_check"
                                    id="stock_check" {{$stockVerify == 1?'checked':''}}
                                    data-modal-id="toggle-modal"
                                    data-toggle-id="stock_check"
                                    data-on-image="guest-checkout-on.png"
                                    data-off-image="guest-checkout-off.png"
                                    data-on-title="{{translate('by_Turning_ON_Stock_Check_Mode')}}"
                                    data-off-title="{{translate('by_Turning_Off_Stock_Check_Mode')}}"
                                    data-on-message="<p>{{translate('User_cannot_place_an_order_if_stock_is_not_available.')}}</p>"
                                    data-off-message="<p>{{translate('user_can_place_an_order_if_stock_is_not_availble')}}</p>">
                                <span class="switcher_control"></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <input type="hidden" name="stock_validation_refactor_enabled"
                        value="{{ is_null($stockValidationRefactorEnabled) || (int)$stockValidationRefactorEnabled === 1 ? 1 : 0 }}">
                    <input type="hidden" name="stock_validation_refactor_mirror_mode"
                        value="{{ (int)$stockValidationMirrorMode === 1 ? 1 : 0 }}">

                    @php($autoSearchLimit=getWebConfig('character_trigger_limit_for_autosearch'))
                    <div class="col-xl-4 col-md-6">
                        <div class="form-group">
                            <label class="title-color text-capitalize" for="refund_day_limit">{{translate('Character_Trigger_Limit')}}
                                <span class="input-label-secondary cursor-pointer" data-toggle="tooltip" data-placement="top"
                                    title="{{translate('The_search_response_will_appear_only_after_the_user_types_a_predefined_number_of_characters')}}">
                                    <img width="16" src="{{dynamicAsset(path: 'public/assets/back-end/img/info-circle.svg')}}" alt="">
                                </span>
                            </label>
