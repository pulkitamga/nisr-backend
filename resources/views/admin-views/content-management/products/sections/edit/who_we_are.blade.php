...
@section('title', translate('Edit Who We Are Section'))
...
<div class="content container-fluid">
    <div class="card">
        <div class="card-header">
            <h2 class="h1 text-capitalize">{{ translate('Edit Who We Are Section') }}</h2>
        </div>
        <div class="card-body">
            <form
                action="{{ route('admin.content-management.about-us.update', ['section' => 'who_we_are', 'id' => $model->id]) }}"
...
<ul class="nav nav-tabs mb-4">
                ...
                <div class="form-group {{ $lang != $activeLanguage ? 'd-none' : '' }} form-system-language-form"
                    ...
                    <!-- Content -->
                    <label class="mt-3">{{ translate('Content') }} ({{ strtoupper($lang) }})</label>
                    <textarea name="content[]" rows="5"
                        class="form-control">{{ $lang == $defaultLanguage ? $model->content : ($translations[$lang]['content'] ?? '') }}</textarea>
