            ...
            <div class="chart-col">
                @php
                    $stageTitle = match ($filters['date_type'] ?? 'this_year') {
                        ...
                        'this_week' => translate('deal_stage_mix') . ' (' . $snapshotFrom->format('M d') . ' - ' . $snapshotTo->format('M d, Y') . ')',
                        'this_month' => translate('deal_stage_mix') . ' (' . $snapshotFrom->format('F Y') . ')',
                        'this_year' => translate('deal_stage_mix') . ' (' . $snapshotFrom->format('Y') . ')',
                        'custom_date' => translate('deal_stage_mix') . ': ' . $snapshotFrom->format('j F Y') . ' - ' . $snapshotTo->format('j F Y'),
    ...
    <div class="full-chart">
        @php
            $statusTitle = match ($filters['date_type'] ?? 'this_year') {
                ...
                'this_week' => translate('message_status_distribution') . ' (' . $snapshotFrom->format('M d') . ' - ' . $snapshotTo->format('M d, Y') . ')',
                'this_month' => translate('message_status_distribution') . ' (' . $snapshotFrom->format('F Y') . ')',
                'this_year' => translate('message_status_distribution') . ' (' . $snapshotFrom->format('Y') . ')',
                'custom_date' => translate('message_status_distribution') . ': ' . $snapshotFrom->format('j F Y') . ' - ' . $snapshotTo->format('j F Y'),
                default => translate('message_status_distribution') . ' (' . translate('last_12_months') . ')',
            };
        @endphp
        <div class="chart-title">{{ $statusTitle }}</div>
        <img src="{{ $statusChart }}" style="width:100%; max-height:200px; object-fit:contain;" alt="Message Status" />
    </div>
@endif
...
@if(!empty($insights))
    <div class="insights-box">
        <div class="chart-title">{{ translate('insights') }}</div>
        <ol>
...
<div class="table-container">
    ...
    <table>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="5" style="text-align: center; padding: 15px;">{{ translate('no_owner_activity_in_this_period') }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
