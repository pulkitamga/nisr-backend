    ...
    <div class="content container-fluid crm-insights-page {{ $isRtl ? 'text-end' : '' }}" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
        <div class="report-hero mb-3">
            <div class="d-flex flex-wrap justify-content-between gap-2 align-items-center">
                ...
                <span class="badge badge-light text-dark">{{ translate('updated') }}
                    {{ now()->locale(app()->getLocale())->translatedFormat('d M Y h:i A') }}</span>
            </div>
        </div>
            ...
            <div class="col-xl-8">
                <div class="card h-100">
                    <div class="card-header border-0 d-flex justify-content-between align-items-center">
                        <h4 class="mb-0">{{ translate('crm_trend') }}
                            ({{ $snapshotFrom->locale(app()->getLocale())->translatedFormat('d M Y') }} -
                            {{ $snapshotTo->locale(app()->getLocale())->translatedFormat('d M Y') }})</h4>
            ...
            <div class="col-xl-4">
                <div class="card h-100">
                    <div class="card-header border-0 d-flex justify-content-between align-items-center">
                        ...
                        <span
                            class="badge-soft">{{ $snapshotFrom->locale(app()->getLocale())->translatedFormat('d M Y') }}
                            -
                            {{ $snapshotTo->locale(app()->getLocale())->translatedFormat('d M Y') }} </span>
                    </div>
                    <div class="card-body">
        ...
        <div class="card">
            <div class="card-header border-0">
                ...
                <h4 class="mb-0">{{ translate('top_deal_owners_by_value') }}
                    ({{ $snapshotFrom->locale(app()->getLocale())->translatedFormat('d M Y') }} -
                    {{ $snapshotTo->locale(app()->getLocale())->translatedFormat('d M Y') }})</h4>
            </div>
