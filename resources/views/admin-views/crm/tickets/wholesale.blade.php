...
<div class="content container-fluid">
    ...
    $statusId = request()->has('status') ? request()->input('status') : (string) \App\Support\WholesaleTicketWorkflow::STATUS_NEW;
    ...
    $headerActions = [
        [
            'type' => 'export',
            'url' => route('admin.support-ticket.export', 'wholesale'),
            'form_id' => 'crm-wholesale-ticket-toolbar',
    ...
    @include('admin-views.crm.partials._list-toolbar', [
        'toolbarId' => 'crm-wholesale-ticket-toolbar',
        'toolbarAction' => url()->current(),
        'toolbarResetUrl' => route('admin.support-ticket.view', 'wholesale'),
        'toolbarFields' => $toolbarFields,
        'toolbarSummary' => $toolbarSummary,
    ])
    <div class="card">
        @include('admin-views.crm.partials._list-card-header', [
            'listHeaderTitle' => translate('Wholesale Ticket'),
                            ...
                            <div class="crm-row-actions">
                                ...
                                <div class="dropdown crm-row-actions__menu">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        ...
                                        <a class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#showWholesaleFollowUpModal" data-ticket-id="{{ $ticket->id }}" data-department-id="{{ $ticket->department_id }}" data-employee-id="{{ $ticket->employee_id }}" data-status-id="{{ $ticket->status }}" data-status-name="{{ $ticket->status_details?->name ?? '' }}" data-follow-up-date="{{ $ticket->follow_up_date ? \Carbon\Carbon::parse($ticket->follow_up_date)->format('Y-m-d') : '' }}" title="{{ translate('Follow-up details') }}">
                                            {{ translate('follow_Up') }}
                                        </a>
                                        @endif
