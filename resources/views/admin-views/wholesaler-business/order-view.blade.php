                            ...
                            <div id="product_dropdown_wrapper" class="mt-2 hidden">
                                <select id="product_select" class="js-example-matcher w-64"
                                    ...
                                    <option value="{{ $wholesale->id }}"
                                        ...
                                        data-variation-type="{{ $wholesale->variation_type }}" data-name="{{ $wholesale->product->name }}"
                        ...
                        <table class="table table-hover table-borderless table-thead-bordered min-w-full divide-y divide-gray-200 border rounded-lg overflow-hidden">
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="px-2 py-2 text-left">{{ translate('Requested Qty') }}</th>
                            ...
                            <tbody id="product_table_body" class="bg-white divide-y divide-gray-200">
                                ...
                                <tr data-product-id="{{ $item->product_id }}" data-variation-type="{{ $item->product_variation_type }}">
                                    <td class="px-4 py-2">
                                        {{ $item->product->name }} ({{ $item->product_variation_type ?? 'No Variation' }})
                                    </td>
   ...
   $(productSelect).on('select2:select', function(e) {
    ...
    if (isDuplicate) {
        Swal.fire({
            icon: 'warning',
            title: 'Oops...',
            text: 'This product variation is already added.',
    ...
    window.addCharge = function() {
        ...
        container.innerHTML = `
    <input type="text" name="charges[${chargeIndex}][name]" placeholder="Charge Name"
        class="flex-1 px-3 py-2 border rounded" />

    <input type="number" name="charges[${chargeIndex}][value]" placeholder="Value"
        class="px-3 py-2 border rounded" data-charge data-fieldname="Charge Value"
...
`;
    ...
    window.addDiscount = function() {
        ...
        container.classList.add('flex', 'gap-2', 'items-center', 'mt-2');
        container.innerHTML = `
    <input type="text" name="discounts[${discountIndex}][name]" placeholder="Discount Name"
        class="flex-1 px-3 py-2 border rounded" />

    <input type="number" name="discounts[${discountIndex}][value]" placeholder="Value"
        class="px-3 py-2 border rounded" data-discount data-fieldname="Discount Value"
        oninput="updateFinalPrice()" required />
    ...
    document.getElementById('submit_btn').addEventListener('click', function(event) {
        ...
        if (emptyField) {
            Swal.fire({
                ...
                text: `Field "${emptyField.dataset.fieldname || emptyField.previousElementSibling?.innerText || emptyField.name}" is required.`,
            }).then(() => {
                emptyField.focus();
            });
