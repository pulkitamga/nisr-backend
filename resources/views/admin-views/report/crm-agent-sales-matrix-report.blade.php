    ...
    <div class="content container-fluid {{ $isRtl ? 'text-right' : 'text-left' }}">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-control" name="date_range" id="date_range">
                                ...
                                <option value="this_week"
                                    {{ ($filters['date_range'] ?? '') == 'this_week' ? 'selected' : '' }}>This Week
                                </option>
                                <option value="this_month"
                                    {{ ($filters['date_range'] ?? '') == 'this_month' ? 'selected' : '' }}>This Month
                                </option>
                                <option value="this_year"
                                    {{ ($filters['date_range'] ?? '') == 'this_year' ? 'selected' : '' }}>This Year
                                </option>
                                <option value="custom" {{ ($filters['date_range'] ?? '') == 'custom' ? 'selected' : '' }}>
                                    Custom Range</option>
                            </select>
                        </div>
        ...
        <div class="card mb-3">
            <div class="card-header border-0">
                <h4 class="mb-0">{{ translate('monthly_agent_sales_matrix_retail_wholesale') }}</h4>
            </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            @foreach ($employeesForMatrix as $employee)
                                <th class="matrix-head-agent {{ !$loop->last ? 'agent-separator' : '' }}" colspan="4">
                                    {{ $employee->name }}</th>
                            @endforeach
                        ...
                        <tr>
                            @foreach ($employeesForMatrix as $employee)
                                ...
                                <th class="matrix-head-sub {{ !$loop->last ? 'agent-separator' : '' }}">
                                    {{ translate('wholesale_customers') }}</th>
                            @endforeach
                    ...
                    <tbody>
                        @forelse($monthlyRows as $row)
                            <tr>
                                ...
                                @endforeach
                                <td class="text-center font-weight-bold totals-separator">
                                    {{ $row->totals['total_batteries'] }}</td>
                        ...
                        <tfoot>
                            <tr class="table-active font-weight-bold">
                                ...
                                @foreach ($employeesForMatrix as $employee)
                                    ...
                                    <td class="text-center {{ !$loop->last ? 'agent-separator' : '' }}">
                                        {{ $employeeTotal->wholesale_customers ?? 0 }}</td>
                                @endforeach
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm mb-0">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-3">{{ translate('no_data_found') }}
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
