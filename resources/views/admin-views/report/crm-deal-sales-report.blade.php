                ...
                <form method="GET" action="{{ url()->current() }}">
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-12 d-flex flex-wrap gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.report.crm-sales-performance-export-pdf') }}"
                                id="pdfExportForm" class="d-inline">
                                @csrf
                                ...
                                <input type="hidden" name="employee_chart" id="employee_chart">
                                <input type="hidden" name="status_chart" id="status_chart">
                                <input type="hidden" name="sales_chart" id="sales_chart">
...
@push('script')
    ...
    <script>
        ...
        document.getElementById('downloadPdf').addEventListener('click', function() {
            ...
            const employeeChart = ApexCharts.getChartByID("crm-employee-status-chart");
            const statusChart = ApexCharts.getChartByID("crm-status-split-chart");
            const salesChart = ApexCharts.getChartByID("crm-employee-sales-type-chart");

            Promise.all([
                employeeChart ? employeeChart.dataURI() : null,
                statusChart ? statusChart.dataURI() : null,
                salesChart ? salesChart.dataURI() : null
            ]).then((results) => {

                if (results[0]) document.getElementById('employee_chart').value = results[0].imgURI;
                if (results[1]) document.getElementById('status_chart').value = results[1].imgURI;
                if (results[2]) document.getElementById('sales_chart').value = results[2].imgURI;
