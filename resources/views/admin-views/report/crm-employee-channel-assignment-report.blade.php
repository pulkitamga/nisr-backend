                    ...
                    <div class="row g-2 align-items-start">
                        ...
                        <div class="col-md-2">
                            ...
                            <select class="form-control" name="date_range" id="date_range">
                                ...
                                <option value="this_week" {{ request('date_range') == 'this_week' ? 'selected' : '' }}>This
                                    Week</option>
                                <option value="this_month" {{ request('date_range') == 'this_month' ? 'selected' : '' }}>
                                    This Month</option>
                                <option value="this_year" {{ request('date_range') == 'this_year' ? 'selected' : '' }}>This
                                    Year</option>
        ...
        <div class="row g-2 mb-3">
            <div class="col-12">
                <div class="report-chart-card">
                    <h4 class="mb-2">{{ translate('monthly_assigned_interactions_by_channel') }} ({{ $dateRange }})
                    </h4>
                ...
                <table class="table table-bordered matrix-table mb-0">
                        ...
                        <tr>
                          <th class="matrix-sticky" rowspan="2">{{ translate('period') }}</th>
                            @foreach ($employeesForMatrix as $employee)
                                <th class="matrix-head-agent {{ !$loop->last ? 'agent-separator' : '' }}" colspan="3">
                                    {{ $employee->name }}</th>
                            @endforeach
                    ...
                    <tbody>
                        @forelse($monthlyRows as $row)
                            <tr>
                             <td class="matrix-sticky">{{ $periodLabel }}</td>
                                @foreach ($employeesForMatrix as $employee)
            ...
            <div class="table-responsive">
                <table class="table table-borderless table-thead-bordered table-nowrap card-table mb-0">
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-3">{{ translate('no_data_found') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
...
@push('script')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            toggleCustomDate();
            range.addEventListener('change', toggleCustomDate);
