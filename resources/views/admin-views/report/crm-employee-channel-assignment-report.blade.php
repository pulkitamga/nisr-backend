...
@push('script')
    ...
    <script>
        (function() {
            ...
            const hasData = chartData.series.some(s =>
                s.data.some(v => v > 0)
            );
            ...
            new ApexCharts(el, {
                ...
                xaxis: {
                    categories: hasData ? [...new Set(chartData.labels)] : [],
                    labels: {
                        rotate: -30
                    }
                },
            ...
            $('.btn-outline-danger[href*="export-pdf"]').on('click', function(e) {
                ...
                setTimeout(function() {
                    ...
                    const chartEl = document.querySelector(
                        '#crm-employee-channel-monthly-chart svg');

                    if (chartEl) {
                        try {
                            // Get the chart instance to extract data properly
                            const chartInstance = ApexCharts.getChartByID ?
                                ApexCharts.getChartByID('crm-employee-channel-monthly-chart') :
                                null;

                            // Get the actual chart data from the original data source
                            const chartData = @json($chart);
