...
<div class="__inline-61">
    ...
    @if ($featuredProductsList->count() > 0 && auth('customer')->check() && auth('customer')->user()->user_type == 0 ||
    !auth('customer')->check() )
    ...
    @include('web-views.partials._wholeseller-product')


    @if(auth('customer')->check() && auth('customer')->user()->user_type == 0 || !auth('customer')->check() &&
    $web_config['featured_deals'] && (count($web_config['featured_deals'])>0))
    ...
    <section class="new-arrival-section">
        ...
        <div class="container rtl px-0 px-md-3">
            <div class="row g-3 mx-max-md-0">
                ...
                @if (auth('customer')->check() && auth('customer')->user()->user_type == 0 || !auth('customer')->check()
                && $bestSellProduct->count() >0)
                @include('web-views.partials._best-selling')
                @endif

                @if (auth('customer')->check() && auth('customer')->user()->user_type == 0 || !auth('customer')->check()
                && $topRatedProducts->count() >0)
    ...
    <!-- @if(auth('customer')->check() && auth('customer')->user()->user_type == 0 || !auth('customer')->check() &&
    $web_config['brand_setting'] && $brands->count() > 0)
