    ...
    <div class="container product-cart-option-container">
        <form class="add-to-cart-sticky-form addToCartDynamicForm">
            ...
            @php
                ...
                $stickyVariationRows = json_decode($productDetails->variation, true) ?? [];
                $stickyVariantTokenAvailability = [];
                foreach ($stickyVariationRows as $stickyVariation) {
                    $stickyTokens = explode('-', (string)($stickyVariation['type'] ?? ''));
                    foreach ($stickyTokens as $stickyToken) {
                        $normalizedStickyToken = str_replace(' ', '', trim((string)$stickyToken));
                        if ($normalizedStickyToken === '') {
                            continue;
                        }
                        if (!array_key_exists($normalizedStickyToken, $stickyVariantTokenAvailability)) {
                            $stickyVariantTokenAvailability[$normalizedStickyToken] = false;
                        }
                        if ((int)($stickyVariation['qty'] ?? 0) > 0) {
                            $stickyVariantTokenAvailability[$normalizedStickyToken] = true;
                        }
                    }
                }
            @endphp
                ...
                <div class="border-bottom d-flex flex-column gap-3 mb-3 pb-3">
                    ...
                    <div>
                        ...
                        <div class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 flex-start ps-0 overflow-x-auto flex-nowrap overflow-y-hidden scrollbar-none">
                            @php($isDefaultOptionSelected = false)
                            @foreach ($choice->options as $index => $option)
                            @php($normalizedOption = str_replace(' ', '', trim((string)$option)))
                            @php($isOptionOutOfStock = $stockCheckStatus == 1 && count($stickyVariationRows) > 0 && !($stickyVariantTokenAvailability[$normalizedOption] ?? false))
                            @php($shouldBeChecked = !$isDefaultOptionSelected && !$isOptionOutOfStock)
                            <div class="user-select-none">
                                <div class="for-mobile-capacity">
                                    <input type="radio"
                                        id="sticky-{{ str_replace(' ', '', ($choice->name. '-'. $option)) }}"
                                        name="{{ $choice->name }}" value="{{ $option }}"
                                        @if($shouldBeChecked) checked @php($isDefaultOptionSelected = true) @endif
                                        @if($isOptionOutOfStock) disabled @endif>
                                    <label class="__text-12px max-content {{ $isOptionOutOfStock ? 'text-muted opacity-50' : '' }}"
                    ...
                    <div class="media-body">
                        ...
                        <div class="d-flex flex-wrap align-items-center mb-2 pro">
                            ...
                            <div class="d-none d-sm-flex flex-wrap align-items-center">
                                ...
                                <span class="{{ count(json_decode($productDetails->variation ?? '[]', true)) > 0 ? '__inline-25 mx-2' : '' }} mt-0"></span>
                                <span class="fs-12">
                ...
                <div class="d-flex align-items-center gap-2 gap-sm-3 gap-xl-4">
                    <div class="d-flex justify-content-between align-items-center quantity-box quantity-box-120 border rounded border-base web-text-primary">
                        ...
                        <input type="number" name="quantity"
                            ...
                            max="{{$productDetails['product_type'] == 'physical' && $stockCheckStatus == 1 ? $productDetails->current_stock : DEFAULT_STOCK}}">
