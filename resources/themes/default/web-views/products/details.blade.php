                                ...
                                <form class="mb-2 addToCartDynamicForm add-to-cart-details-form">
                                    ...
                                    <div
                                        ...
                                        @php
                                            ...
                                            $qty = 0;
                                            $variationRows = json_decode($product->variation, true) ?? [];
                                            $variantTokenAvailability = [];
                                            if(!empty($product->variation)){
                                            foreach ($variationRows as $variation) {
                                                    $tokens = explode('-', (string)($variation['type'] ?? ''));
                                                    foreach ($tokens as $token) {
                                                        $normalizedToken = str_replace(' ', '', trim((string)$token));
                                                        if ($normalizedToken === '') {
                                                            continue;
                                                        }
                                                        if (!array_key_exists($normalizedToken, $variantTokenAvailability)) {
                                                            $variantTokenAvailability[$normalizedToken] = false;
                                                        }
                                                        if ((int)($variation['qty'] ?? 0) > 0) {
                                                            $variantTokenAvailability[$normalizedToken] = true;
                                                        }
                                                    }
                                            ...
                                            <div>
                                                <div class="list-inline checkbox-alphanumeric checkbox-alphanumeric--style-1 mb-0 mx-1 flex-start row ps-0">
                                                    ...
                                                    @foreach ($choice->options as $index => $option)
                                                        ...
                                                        @php($isOptionOutOfStock = $stockCheckStatus == 1 && count($variationRows) > 0 && !($variantTokenAvailability[$normalizedOption] ?? false))
                                                        ...
                                                        <div class="user-select-none">
                                                            <div class="for-mobile-capacity">
                                                                ...
                                                                <label class="__text-12px {{ $isOptionOutOfStock ? 'text-muted opacity-50' : '' }}"
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div class="d-flex justify-content-center align-items-center quantity-box border rounded border-base web-text-primary">
                                                    ...
                                                    <input type="text" name="quantity"
                                                           ...
                                                           max="{{$product['product_type'] == 'physical' && $stockCheckStatus == 1 ? $product->current_stock : DEFAULT_STOCK}}">
                                                    <span class="input-group-btn">
                                                        <button class="btn btn-number __p-10 web-text-primary" type="button"
                                                                data-producttype="{{ $product->product_type }}"
