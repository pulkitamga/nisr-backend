...
@section('title',translate($data['offer_type']).' '.translate('wholesale_products'))
        ...
        <div>
            <h5 class="font-semibold mb-1 text-capitalize">
                <span class="current-product-type" data-all="{{ translate('all') }}"
                    ...
                    {{ translate(str_replace('_',' ',$data['offer_type'])) }} {{ request('product_type') == 'digital' ?
                    translate(request('product_type')) : ''}}
                </span>
                {{ translate('wholesale_products') }} {{ isset($data['brand_name']) ? '('.$data['brand_name'].')' : ''}}
            </h5>
            <div><span class="view-page-item-count clearance-sale-count">{{$products->total()}}</span>
                {{translate('items_found')}}
            </div>
        </div>
        <div class="d-flex flex-wrap gap-3">
            <form id="search-form" class="d-none d-lg-block" action="{{ route('wholesale.products') }}" method="GET">
            ...
            <form class="d-none d-lg-block" action="{{ route('wholesale.products') }}" method="GET">
                <div class="sorting-item">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                        <path d="M11.6667 7.80078L14.1667 5.30078L16.6667 7.80078" stroke="#D9D9D9" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round" />
    ...
    <div class="row">
                ...
                <div class="pb-0 shop-sidebar-scroll">
                    <div class="d-flex gap-3 flex-column">
                        ...
                        <div class="d-lg-none">
                            ...
                            <form id="search-form" action="{{ route('wholesale.products') }}" method="GET">
                                    ...
                                    <div class="card-header flex-between">
                                        <div>
                                            <label class="for-hover-label cursor-pointer get-view-by-onclick"
                                                data-link="{{ route('wholesale.products',['category_id'=> $category['id'],'data_from'=>'category', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '', 'page'=>1]) }}">
                                                {{$category['name']}}
                                            </label>
                                        </div>
                                            ...
                                            <div class="for-hover-label card-header flex-between">
                                                <div>
                                                    <label class="cursor-pointer get-view-by-onclick"
                                                        data-link="{{ route('wholesale.products',['sub_category_id'=> $child['id'],'data_from'=>'category', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '','page'=>1]) }}">
                                                        {{$child['name']}}
                                                    </label>
                                                </div>
                                                <div class="px-2 cursor-pointer menu--caret">
                                            ...
                                            <div class="card-body p-0 ms-2 d--none" id="collapse-{{$child['id']}}">
                                                ...
                                                <div class="card-header">
                                                    <label
                                                        ...
                                                        data-link="{{ route('wholesale.products',['sub_sub_category_id'=> $ch['id'],'data_from'=>'category','page'=>1]) }}">
                                                        {{$ch['name']}}
                                                    </label>
                                                </div>
                                                @endforeach
                            ...
                            <ul class="__brands-cate-wrap attribute-list" data-simplebar
                                ...
                                <ul class="brand mt-2 p-0 for-brand-hover {{Session::get('direction') === "rtl"
                                    ...
                                    <li class="flex-between get-view-by-onclick cursor-pointer"
                                        data-link="{{ route('wholesale.products',['brand_id'=> $brand['id'],'data_from'=>'brand', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '', 'page'=>1]) }}">
                            ...
                            <ul class="__brands-cate-wrap attribute-list" data-simplebar
                                ...
                                <ul class="brand mt-2 p-0 for-brand-hover {{Session::get('direction') === "rtl"
                                    ...
                                    <li class="flex-between get-view-by-onclick cursor-pointer pe-2"
                                        data-link="{{ route('wholesale.products',['publishing_house_id'=> $publishingHouseItem['id'], 'product_type' => 'digital', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '', 'page'=>1]) }}">
                                        <div class="text-start">
                        ...
                        <div class="product-type-digital-section search-product-attribute-container">
                            ...
                            <ul class="__brands-cate-wrap attribute-list" data-simplebar
                                ...
                                <ul class="brand mt-2 p-0 for-brand-hover {{Session::get('direction') === "rtl"
                                    ? 'ms-2' : '' }}" id="brand">
                                    <li class="flex-between get-view-by-onclick cursor-pointer pe-2"
                                        data-link="{{ route('wholesale.products',['author_id' => $productAuthor['id'], 'product_type' => 'digital','offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '', 'page' => 1]) }}">
                                        <div class="text-start">
                                            {{ $productAuthor['name'] }}
                                        </div>
                                        <div class="__brands-cate-badge">
        ...
        <section class="col-lg-9">
            <div class="row" id="ajax-products-view">
                @include('web-views.wholesale._ajax-products',['wholesale.products'=>$products,'decimal_point_settings'=>$decimal_point_settings])
            </div>
        </section>
    </div>
</div>
<span id="products-search-data-backup" data-page="{{ request('page') ?? 1 }}" data-url="{{ route('wholesale.products') }}"
...
<script>
    ...
    $(document).ready(function () {
        ...
        if (selectedMake) {
            ...
            const makeId = makeOption.data('id');
    ...
    $('#model-select').on('change', function () {
        ...
        $('#year-select').prop('disabled', !selectedModel);
    });
</script>
