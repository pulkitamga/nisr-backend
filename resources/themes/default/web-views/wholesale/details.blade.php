    ...
    @php
    $servicePolicy = getWebConfig('service_policy');

    $servicePolicyValue = json_decode($servicePolicy->value ?? '{}', true);

    $translatedContent = getBusinessSettingTranslation(
    'service_policy',
    'value',
    $servicePolicyValue['content'] ?? ''
    );
    @endphp
                    ...
                    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content p-3">
                            ...
                            <form id="serviceRequestForm" action="{{ route('service.request.store') }}" method="POST">
                                @csrf
                                <div class="modal-body request-model-body pt-1" style="max-height: 70vh;">
                                    <div class="p-3">
                                        <h1 class="mb-2 font-size-lg text-center text-danger">{{ $serviceTitle }}</h1>
                                    </div>
                                    <input type="hidden" name="service_id" value="{{ $product->service->id }}">
                                    @if(auth()->guard('customer')->check())
                                    <input type="hidden" name="customer_id" value="{{ auth()->guard('customer')->user()->id }}">
                                    @endif
                                    <input type="hidden" name="vehicle_make" id="vehicle_make_name">

                                    {{-- Price Options --}}
                                    <h6 class="mb-2">{{ translate('Price options') }}</h6>
                                    <div>

                                        <div class="custom-control custom-radio mb-2 padd-input-div">
                                            <input type="radio" id="inShop" name="service_option" class="custom-control-input" value="in_shop" checked>
                                            <label class="custom-control-label radio-input-label" for="inShop">
                                                {{ translate('In-shop') }} : <strong> {{ webCurrencyConverter(amount:$product->service->base_price_inshop) }}</strong>
                                            </label>
                                        </div>
                                    ...
                                    {{-- Service address (only when Mobile selected) --}}
                                    <div class="mb-3" id="mobileAddress" style="display: none;">
                                        <h6 class="mb-2">{{ translate('Service address') }}</h6>
                                        <div class="row">
                                            <div class="form-group col-lg-12">
                                                <label>{{ translate('country') }} <span class="text-danger">*</span></label>
                                                <select id="address-country" class="form-control selectpicker" data-live-search="true" required>
                                            ...
                                            <div class="form-group col-lg-6">
                                                ...
                                                <select id="address-state" class="form-control" required></select>
                                            ...
                                            <div class="form-group col-lg-6">
                                                ...
                                                <select id="address-city" class="form-control" required></select>
                                            ...
                                            <div class="form-group col-lg-6">
                                                ...
                                                <select id="address-area" class="form-control" required></select>
                                    ...
                                    <div class="py-lg-3">
                                        <h6 class="mt-3 mb-2">{{ translate('Vehicle details') }}</h6>
                                        <div class="row g-2">
                                            ...
                                            <div class="col-6 col-md-4">
                                                <label for="makeSelect">{{ translate('Make') }} <span class="input-required-icon">*</span>
                                                </label>
                                                <select id="makeSelect" name="vehicle_make_id" class="form-control" required>
                                                    <option value="" disabled selected>{{ translate('Select_Make') }}</option>
                                            ...
                                            <div class="col-6 col-md-4">
                                                <label for="modelSelect">{{ translate('Model') }} <span class="input-required-icon">*</span>
                                                </label>
                                                <select id="modelSelect" name="vehicle_model" class="form-control" required disabled>
                                                    <option value="" disabled selected>{{ translate('Select Model') }}</option>
                                                    {{-- Options will be populated by JS --}}
                                                </select>
                                            </div>
                                            <div class="col-6 col-md-4">
                                                <label for="yearSelect">{{ translate('Year') }} <span class="input-required-icon">*</span>
                                                </label>
                                            ...
                                            <div class="col-6 col-md-4">
                                                <label class="form-label">{{ translate('Mileage (km)') }} <span class="input-required-icon">*</span>
                                                </label>
                                                <input type="number" name="vehicle_mileage" class="form-control" required>
                                            </div>
                                            <div class="col-6 col-md-4">
                                    ...
                                    <div class="my-2">
                                        <h6 class="my-2">{{ translate('What’s included') }}:</h6>
                                    ...
                                    <hr>
                                    <div class="my-2">
                                        {!! html_entity_decode($translatedContent) !!}
                                    </div>
                                </div>
                                <div class="modal-footer pt-4">
                                    <button type="button" id="confirmServiceRequest" class="btn btn--primary w-100">{{ translate('Confirm Request') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script>
        ...
        document.getElementById('serviceRequestModal').addEventListener('shown.bs.modal', function() {
            fetchAndRenderLocation();
        });

        document.getElementById('confirmServiceRequest').addEventListener('click', function(e) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleAddress() {
                mobileAddress.style.display = mobileOption.checked ? 'block' : 'none';
            }
        ...
        const allModels = @json($makes -> pluck('models', 'id'));
        ...
        makeSelect.addEventListener('change', function() {
            ...
            const makeName = selectedOption.textContent;
    ...
    <script>
        const selectStateLabel = @json(__('Select State'));
        const selectCityLabel = @json(__('Select City'));
        const selectAreaLabel = @json(__('Select Area'));
