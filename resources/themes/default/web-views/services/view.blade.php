...
@section('title',translate($data['offer_type']).' '.translate('services'))
...
<div class="container py-3" dir="{{Session::get('direction')}}">
    <div class="search-page-header">
        <div>
            <h5 class="font-semibold mb-1 text-capitalize">
                <span class="current-product-type" data-all="{{ translate('all') }}"
                    ...
                    {{ translate(str_replace('_',' ',$data['offer_type'])) }} {{ request('product_type') == 'services' ?
                    translate(request('product_type')) : ''}}
                </span>
                {{ translate('services') }} {{ isset($data['brand_name']) ? '('.$data['brand_name'].')' : ''}}
            </h5>
        ...
        <div class="d-flex flex-wrap gap-3">
            <form id="search-form" class="d-none d-lg-block" action="{{ route('services') }}" method="GET">
                    <input type="hidden" name="product_type" value="services">
            ...
            <form class="d-none d-lg-block" action="{{ route('services') }}" method="GET">
                <div class="sorting-item">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
                        <path d="M11.6667 7.80078L14.1667 5.30078L16.6667 7.80078" stroke="#D9D9D9" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round" />
    ...
    <div class="row">
                                    ...
                                    <div class="card-header flex-between">
                                        <div>
                                            <label class="for-hover-label cursor-pointer get-view-by-onclick"
                                                data-link="{{ route('services',['category_id'=> $category['id'],'data_from'=>'category', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '', 'page'=>1]) }}">
                                                {{$category['name']}}
                                            </label>
                                        </div>
                                            ...
                                            <div class="for-hover-label card-header flex-between">
                                                <div>
                                                    <label class="cursor-pointer get-view-by-onclick"
                                                        data-link="{{ route('services',['sub_category_id'=> $child['id'],'data_from'=>'category', 'offer_type' => isset($data['offer_type']) ? $data['offer_type'] : '','page'=>1]) }}">
                                                        {{$child['name']}}
                                                    </label>
                                                </div>
                                                <div class="px-2 cursor-pointer menu--caret">
                                            ...
                                            <div class="card-body p-0 ms-2 d--none" id="collapse-{{$child['id']}}">
                                                ...
                                                <div class="card-header">
                                                    <label
                                                        ...
                                                        data-link="{{ route('services',['sub_sub_category_id'=> $ch['id'],'data_from'=>'category','page'=>1]) }}">
                                                        {{$ch['name']}}
                                                    </label>
                                                </div>
                                                @endforeach
        ...
        <section class="col-lg-9">
            <div class="row" id="ajax-products-view">
                @include('web-views.services._ajax-products',['products'=>$products,'decimal_point_settings'=>$decimal_point_settings])
            </div>
        </section>
    </div>
</div>
<span id="products-search-data-backup" data-page="{{ request('page') ?? 1 }}" data-url="{{ route('services') }}"
