...
<head>
    ...
    <link rel="stylesheet" href="{{ theme_asset(path: 'public/assets/front-end/css/blog/custom.css')  }}">
...
<body class="toolbar-enabled {{ $direction === 'rtl' ? 'rtl' : 'ltr' }}">
    ...
    <span id="route-cart-updateInstalltionCharges" data-url="{{route('cart.updateInstalltionCharges')}}"></span>
<span id="route-cart-updateExchangeCharges" data-url="{{route('cart.updateExchangeCharges')}}"></span>
...
@php($translatedCookieText = getBusinessSettingTranslation('cookie_setting', 'cookie_text', json_decode($web_config['cookie_setting']['value'] ?? '{}', true)['cookie_text'] ?? ''))
        let cookie_content = `
