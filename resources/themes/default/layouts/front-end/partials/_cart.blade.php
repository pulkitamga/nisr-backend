...
$cartRoute = $isLoggedIn ? route('shop-cart') : route('customer.auth.login');
@endphp

<div class="navbar-tool dropdown {{ Session::get('direction') === 'rtl' ? '' : '' }}">
    <a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="{{ $cartRoute }}">
        <span class="navbar-tool-label">{{ $cart->count() }}</span>
        <i class="navbar-tool-icon czi-cart"></i>
    </a>

    <a class="navbar-tool-text text-white ms-2" href="{{ $cartRoute }}">
    ...
    <div class="dropdown-menu cart-dropdown dropdown-menu-{{ session('direction') === " rtl" ? 'left' : 'right' }} __w-20rem
        ...
        <div class="widget widget-cart px-3 pt-2 pb-3">
            ...
            <div class="__h-20rem" data-simplebar data-simplebar-auto-hide="false">
                ...
                function ($query) {return $query->active();}])->first())
                ...
                <div class="widget-cart-item">
                    <div class="media">
                        ...
                        <div
                            ...
                            @if (!$user || $user->user_type != 1)
                            {{-- Guest user ya user_type == 1 wali condition --}}
                            ...
                            @else
                            <div class="__quantity ms-0 mb-4">
            ...
            \App\Utils\OrderManager::getFreeDeliveryOrderAmountArray($cart[0]->cart_group_id))
            ...
            <div class="py-3">
                ...
                <small
                    class="amount_need_to_fullfill {{$free_delivery_status['amount_need'] <= 0 ? 'd-none' :''}}"><span
                        class="text-accent __text-12px free_delivery_amount_need">{{ webCurrencyConverter(amount:
                        $free_delivery_status['amount_need']) }}</span> {{ translate('add_more_for_free_delivery') }}
                </small>
            ...
            <!-- Purchase Order Button for user_type 1 -->
            <form action="{{ route('wholesale.createOrder') }}" method="POST">
                ...
                @endforeach
                <button type="submit" class="btn btn--primary btn-sm btn-block font-bold rounded text-capitalize">
                    {{ translate('purchase_order') }}
                </button>
            </form>
            @else
            <!-- Regular Checkout Button for other user types -->
