...
let productListPageData = {
    ...
    product_type: productListPageBackup.data('product-type') || 'physical',
    author_id: productListPageBackup.data('author-id'),
    publishing_house_id: productListPageBackup.data('publishing-house-id'),
    make: productListPageBackup.data('make') || '',
    model: productListPageBackup.data('model') || '',
    year: productListPageBackup.data('year') || '',
    rating: [],
};
...
$('#make-select, #model-select, #year-select').on('change', function () {
    productListPageData.make = $('#make-select').val();
    productListPageData.model = $('#model-select').val();
    productListPageData.year = $('#year-select').val();
    getProductListFilterRender();
});
