...
function renderServiceQuickView() {
    $(".action-service-quick-view").on("click", function () {
        let serviceId = $(this).data("service-id");
        serviceQuickView(serviceId);
    });
}

function serviceQuickView(service_id) {
    $.get({
        url: $("#route-service-quick-view").data("url"), // set via hidden span or meta
        dataType: "json",
        ...
        success: function (data) {
            $("#service-quick-view-modal").html(data.view);
            $("#service-quick-view").modal("show");
        },
        complete: function () {
            $("#loading").hide();
        }
...
function cartListQuantityUpdateInit() {
    $(".action-update-cart-quantity")
        .off("click.cartListQty")
        .on("click.cartListQty", function () {
    ...
    $(".action-change-update-cart-quantity-list")
        .off("change.cartListQty")
        .on("change.cartListQty", function () {
        let minimumOrderQuantity = $(this).data("minimum-order");
        let key = $(this).data("cart-id");
        let increment = $(this).data("increment");
        let event = $(this).data("event");
...
function actionCheckoutFunctionInit() {
    $(document)
        .off("click.checkoutAction", ".action-checkout-function")
        .on("click.checkoutAction", ".action-checkout-function", function () {
    ...
    $(document)
        .off("change.checkoutAction", ".action-set-shipping-id")
        .on("change.checkoutAction", ".action-set-shipping-id", function () {
...
function checkValidityForVariantPrice(formSelector) {
    let quantityInput = $(formSelector).find("input[name=quantity]");
    let quantityValue = parseFloat(quantityInput.val() || 0);
    let quantityMax = parseFloat(quantityInput.attr("max") || 0);
    return quantityValue > 0 && quantityMax > 0 && checkAddToCartValidity(formSelector);
}
...
function updateProductDetailsTopSection(formSelector, response) {
    ...
    if (response?.product_type?.toString() === 'physical') {
        ...
        if (response?.quantity <= 0) {
            ...
            productRestockRequestButton.removeAttr('disabled');
            $(formSelector).find(".product-add-and-buy-section").hide().removeClass('d-flex');
            $(formSelector).find(".btn-number[data-field='quantity']").attr("disabled", true);
            $(formSelector).find(".product-details-cart-qty").val(0);
        } else {
            $(formSelector).find(".product-restock-request-section").hide();
            $(formSelector).find(".product-add-and-buy-section").show().addClass('d-flex');
            $(formSelector).find(".btn-number[data-field='quantity']").attr("disabled", false);
        }
        if (response?.restock_request_status) {
            productRestockRequestButton.html(productRestockRequestButton.data('requested'));
            productRestockRequestButton.attr('disabled', true);
    ...
    if (parseFloat($(formSelector).find(".product-details-cart-qty").attr("max") || 0) <= 0) {
        $(formSelector).find(".btn-number[data-type='minus'][data-field='quantity']").attr("disabled", true);
    } else if ($(formSelector).find(".product-details-cart-qty").attr("min") < parseFloat($(formSelector).find(".product-details-cart-qty").val())) {
...
//             $('#desktop-search-bar').toggleClass('d-none');
//         });
//     });

$(document).ready(function () {
    $('#desktop-search-toggle').on('click', function () {
        $('#desktop-search-bar').toggleClass('active');
    });

    $('.close-search-form').on('click', function () {
        $('#desktop-search-bar').removeClass('active');
    });
});
