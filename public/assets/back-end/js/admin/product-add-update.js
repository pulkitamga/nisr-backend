...
function bindImageFallbackOnError() {
    let fallbackImage =
        $("#image-path-of-product-upload-icon-two").data("path") ||
        $("#image-path-of-product-upload-icon").data("path");

    $(".onerror-add-class-d-none")
        .off("error")
        .on("error", function () {
            let $img = $(this);
            if (fallbackImage && $img.attr("src") !== fallbackImage) {
...
function getProductTypeFunctionality() {
    ...
    else if (productType && productType.toString() === "services") {
        elementProductColorSwitcherByIDFunctionality("reset");
        $("#digital_product_type_show").hide();
        $("#digital_file_ready").val("");
