...
class BusinessSettingsController extends BaseController
{
    ...
    public function getView(): View
    {
        ...
        $businessSetting['company_name_translations'] = $this->getLanguageWiseConfigValues(
            value: $this->getSettings(object: $web, type: 'company_name')->value ?? '',
            languages: $languages,
            defaultLanguage: $defaultLanguage
        );
        $businessSetting['shop_address_translations'] = $this->getLanguageWiseConfigValues(
            value: $this->getSettings(object: $web, type: 'shop_address')->value ?? '',
            languages: $languages,
            defaultLanguage: $defaultLanguage
        );
    ...
    public function updateSettings(BusinessSettingRequest $request, BusinessSettingService $businessSettingService): RedirectResponse
    {
        ...
        $shopAddress = $this->prepareLanguageWiseData(
            languages: $request->input('basic_lang', []),
            values: $request->input('shop_address', []),
            defaultLanguage: $defaultLanguage
        );
        $copyrightText = $this->prepareLanguageWiseData(
            languages: $request->input('text_lang', []),
    ...
    public function getCacheSettingsView(): View
    {
        return view(BusinessSettings::CACHE[VIEW]);
    }
    ...
    public function getWarrantySettingsView(): View
    {
        $enablePublicForm = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_enable_public_form']);
        $requireOtp = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_require_otp']);
        $autoApproveOffPlatform = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_auto_approve_off_platform']);
        $captchaEnabled = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_captcha_enabled']);
        $agentEmail = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_agent_email']);
        $slaFirstResponse = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_sla_first_response']);
        $slaDecision = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_sla_decision']);
        $rateLimitWindow = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_rate_limit_window']);
        $warrantyMonths = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_months']);
        $rateLimitMaxAttempts = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'warranty_rate_limit_max_attempts']);
        return view(BusinessSettings::WARRANTY_SETTINGD[VIEW], compact(
            'enablePublicForm',
            'requireOtp',
            'autoApproveOffPlatform',
            'captchaEnabled',
            'agentEmail',
            'slaFirstResponse',
            'slaDecision',
    ...
    public function updateWarrantySettings(Request $request): RedirectResponse
    {
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_enable_public_form', value: $request->get('enable_public_form', 0));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_require_otp', value: $request->get('require_otp', 0));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_auto_approve_off_platform', value: $request->get('auto_approve_off_platform', 0));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_captcha_enabled', value: $request->get('captcha_enabled', 0));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_agent_email', value: $request->get('agent_email', ''));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_sla_first_response', value: $request->get('sla_first_response', 1));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_sla_decision', value: $request->get('sla_decision', 3));
        $this->businessSettingRepo->updateOrInsert(type: 'warranty_rate_limit_window', value: $request->get('rate_limit_window', 2));
    ...
    {
        $lock = Cache::lock('admin:clear-cache', 90);

        if (!$lock->get()) {
            Toastr::warning(translate('cache_clear_is_already_running'));
            return back();
        }
        ...
        try {
            ...
            $this->deleteFilesExceptGitignore(storage_path('framework/cache/data'));
            $this->deleteFilesExceptGitignore(storage_path('framework/sessions'));
            $this->deleteFilesExceptGitignore(storage_path('debugbar'));
            $this->deleteFilesExceptGitignore(base_path('bootstrap/cache'));
    ...
    private function deleteFilesExceptGitignore(string $directory): void
    {
        if (!is_dir($directory)) {
            return;
        }
