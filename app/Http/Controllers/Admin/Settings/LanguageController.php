...
class LanguageController extends BaseController
{
    ...
    public function add(LanguageRequest $request, LanguageService $languageService): RedirectResponse
    {
        ...
        $existingLanguageList = json_decode($language['value'], true);
        $newCode = strtolower($request['code']);
        foreach ($existingLanguageList as $item) {
            if (($item['code'] ?? '') === $newCode) {
                Toastr::error(translate('language_code_already_exists'));
                return back();
            }
        }
    ...
    public function update(LanguageRequest $request, LanguageService $languageService): RedirectResponse
    {
        ...
        $newCode = strtolower($request['code']);
        foreach ($existingLanguageList as $item) {
            $itemCode = strtolower($item['code'] ?? '');
            if ($itemCode === $newCode && $itemCode !== $oldCode) {
                Toastr::error(translate('language_code_already_exists'));
                return back();
            }
        }
        ...
        $this->businessSettingRepo->updateOrInsert(type: 'language', value: $languageArray['languages']);
        $this->businessSettingRepo->updateOrInsert(type: 'pnc_language', value: json_encode($languageArray['codes']));
        clearWebConfigCacheKeys();
