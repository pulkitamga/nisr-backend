...
class OrderController extends BaseController
{
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        if ($order) {
            ...
            $activeBranches = $this->branchRepo->getListWhere(filters: ['status' => 'active'], dataLimit: 'all');
            $orderProductIds = $order->details->pluck('product_id')->toArray();
            $branchWiseProductStock = [];
            $branches = [];
            ...
            foreach ($activeBranches as $branch) {
                ...
                foreach ($order->details as $detail) {
                    ...
                    $variantRaw = $detail->variant;
                    $stock = 0;

                    $variantString = "";

                    if (!empty($variantRaw) && is_string($variantRaw) && str_starts_with($variantRaw, '{')) {

                        $variantDecoded = json_decode($variantRaw, true);
                        $variantString = trim($variantDecoded['type'] ?? '');
                    } else {
                        $variantString = trim($variantRaw);
                    }
                    ...
                    // save
                    $branchWiseProductStock[$branch->id][$productId] = (int)$stock;
                    $totalBranchStock += (int)$stock;
    ...
    public function updateStatus(
        ...
        if (!$stockUpdated) {
            return response()->json([
                'success' => 0,
                'message' => translate('Stock_not_available_in_selected_branch'),
            ]);
        }
        ...
        $stage = match (strtolower($order['order_status'])) {
        ...
        $fulfillmentStatus = strtolower($order['order_status']) === 'delivered' ? 'fulfilled' : 'unfulfilled';

        if (in_array(strtolower($order['order_status']), ['delivered', 'confirmed', 'pending', 'processing', 'out_for_delivery', 'failed', 'canceled', 'returned'])) {
            \App\Services\CrmService::createOrUpdateDeal($order, $stage, $fulfillmentStatus);
        }

        if (strtolower($order['order_status']) === 'delivered') {
            $this->orderRepo->update(id: $request['id'], data: ['payment_status' => 'paid', 'is_pause' => 0]);
            $this->orderDetailRepo->updateWhere(
                params: ['order_id' => $order['id']],
                data: ['delivery_status' => $order['order_status'], 'payment_status' => 'paid']
            );
        }
        event(new OrderStatusEvent(key: $order['order_status'], type: 'customer', order: $order));
        ...
        if ($loyaltyPointStatus == 1 && !$order['is_guest'] && strtolower($order['order_status']) === 'delivered') {
            $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(
                userId: $order['customer_id'],
                reference: $order['id'],
                amount: usdToDefaultCurrency(amount: $order['order_amount'] - $order['shipping_cost']),
        ...
        if (!$order['is_guest'] && $refEarningStatus == 1 && strtolower($order['order_status']) === 'delivered') {
        ...
        if ($order['delivery_man_id'] && strtolower($order['order_status']) === 'delivered') {
            ...
            if (empty($deliverymanWallet)) {
                $deliverymanWalletData = $deliveryManWalletService->getDeliveryManData(
                    id: $order['delivery_man_id'],
                    deliverymanCharge: $order['deliveryman_charge'],
                ...
                $deliverymanWalletData = [
                    ...
                    'cash_in_hand' => $deliverymanWallet['cash_in_hand'] + $cashInHand,
                ];
                $this->deliveryManWalletRepo->updateWhere(params: ['delivery_man_id' => $order['delivery_man_id']], data: $deliverymanWalletData);
            }

            if ($order['deliveryman_charge'] && strtolower($order['order_status']) === 'delivered') {
                $deliveryManTransactionData = $deliveryManTransactionService->getDeliveryManTransactionData(
                    amount: $order['deliveryman_charge'],
                    addedBy: 'admin',
                    id: $order['delivery_man_id'],
                    transactionType: 'deliveryman_charge'
                );
        ...
        if (isset($transaction) && $transaction['status'] === 'disburse') {
            return response()->json($order['order_status']);
        }

        if (strtolower($order['order_status']) === 'delivered' && $order['seller_id'] != null) {
            $this->orderRepo->manageWalletOnOrderStatusChange(order: $order, receivedBy: 'admin');
        }

        return response()->json($order['order_status']);
    }
    public function updateTransferDeliverBranch(Request $request): JsonResponse
    {
        ...
        foreach ($order->details as $detail) {
            ...
            // Detect variant type
            if (!empty($variantRaw) && str_starts_with($variantRaw, '{')) {
                $decoded = json_decode($variantRaw, true);
                $variantString = trim($decoded['type'] ?? '');
            } else {
                $variantString = trim($variantRaw);
            }
        ...
        $this->orderRepo->update(
            id: $request->id,
            data: ['transfer_from_branch' => $selectedBranchId]
        );
    ...
    public function updateDeliverInfo(Request $request, BostaService $bostaService): RedirectResponse
    {
        $order = $this->orderRepo->getFirstWhere(
        ...
        // ✅ BOSTA AUTO-INTEGRATION
        if (strtolower($request['delivery_service_name']) === 'bosta') {
            ...
            try {
                $bostaResponse = $bostaService->createShipment($order);

                // ✅ CORRECT WAY: Extract from data.trackingNumber
            ...
            $updateData['third_party_delivery_tracking_id'] =
                $request['third_party_delivery_tracking_id'];
        }
    ...
    public function updateExchangeInfo(Request $request): RedirectResponse
    {
        $updateOrderData = [
            'exchange_status' => 1,
            'exchange_product_info' => $request['exchange_product_info'],
    ...
    public function updatePaymentStatus(Request $request): JsonResponse
    {
        ...
        if ($order['payment_status'] == 'paid' && $request['payment_status'] == 'unpaid') {
        ...
        // Determine Deal stage based on order_status and payment_status
        $stage = match ($order['order_status']) {
            'delivered' => 'Won/Closed',
            'confirmed' => 'Confirmed Order',
            'pending' => 'Pending Order',
            'processing' => 'Processing Order',
        ...
        // Override stage for unpaid status
        if ($request['payment_status'] == 'unpaid') {
            $stage = in_array($order['order_status'], ['failed']) ? 'Failed Payment' : 'Pending Order';
        }

        $fulfillmentStatus = $order['order_status'] == 'delivered' ? 'fulfilled' : 'unfulfilled';
        \App\Services\CrmService::createOrUpdateDeal($order, $stage, $fulfillmentStatus);
