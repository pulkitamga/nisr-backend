...
class DashboardController extends BaseController
{
    ...
    public function getOrderStatusData(): array
    {
        ...
        $serviceQuery = $this->productRepo->getListWhere(filters: ['product_type' => 'services'], dataLimit: 'all');
        $productQuery = $this->productRepo->getListWhere(filters: ['product_type' => 'physical'], dataLimit: 'all');
        ...
        return [
            ...
            'services' => self::getCommonQueryOrderStatus($serviceQuery),
    ...
    public function getRealTimeActivities(): JsonResponse
    {
        ...
        $leadNotifications = LeadNotification::where('user_id', auth('admin')->id())
            ->where('status', 0)
            ->latest()
            ->take(3)
            ->get(['id', 'title', 'message', 'related_id']); 
