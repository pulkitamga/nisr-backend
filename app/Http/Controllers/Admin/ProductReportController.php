...
class ProductReportController extends Controller
{
    public function all_product(Request $request)
    {
        ...
        $product_query = Product::with([
            ...
            ->where('product_type', 'physical') // keep this
            ...
            ->when($search, function ($query) use ($search) {
                $query->where(function ($q) use ($search) {
                    $q->where('name', 'like', "%{$search}%")
                        ->where('product_type', 'physical'); // ensure physical filter is preserved here too
                });
            });
        ...
        return view('admin-views.report.all-product', compact(
            'sellers',
            'products',
            'chart_data',
            'total_sales_value',
    ...
    public function allProductExportPDF(Request $request)
    {
        $search = $request['search'];
        $from = $request['from'];
        $to = $request['to'];
        ...
        $seller = $request->has('seller_id') && $request['seller_id'] != 'inhouse' && $request['seller_id'] != 'all'
            ? Seller::with('shop')->find($request['seller_id'])
        ...
        $mpdfView = view('admin-views.report.product-report-pdf', ['data' => $data]);
        Helpers::gen_mpdf($mpdfView, 'product_report_', $date_type);
    }
}
