...
class CartController extends BaseController
{
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        $exchangeQuantity = $request['exchange_quantity'] ?? 0;
        $installationQuantity = $request['quantity'] ?? 0;

        $exchangeTotel = $exchangeCharge * $exchangeQuantity;
        $installationTotel = $installationCharge * $installationQuantity;
        ...
        if ($cartId && session()->has($cartId) && count($cartData) > 0) {
            foreach ($cartData as $key => $cartItem) {
                // Safely check only valid cart items (avoid keys like 'customerId')
                if (
                    is_array($cartItem)
                    && isset($cartItem['id'], $cartItem['variant'])
                    && $cartItem['id'] == $request['id']
                    && $cartItem['variant'] == $variant
                ) {
                    if ($variant != null) {
                        $price = $this->cartService->getVariationPrice(
