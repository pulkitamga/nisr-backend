...
class POSControllerOld extends BaseController
{
    ...
    public function getPOSView(object $request): View
    {
        ...
        $products = $this->productRepo->getListWhere(
            ...
            relations: ['clearanceSale' => fn($q) => $q->active()],
            dataLimit: 'all'
        );

        if (!$branchId) {
            $products = $products->filter(fn($p) => $p->current_stock > 0)->values();
        } else {
           $products = $products->map(function ($product) use ($branchId) {
            ...
            Log::info("POS → Product: {$product->name} ({$product->id})");
            Log::info("→ Branch Variations:", $branchVariations);

            $branchOptions = [];

            foreach ($branchVariations as $variation) {

                Log::info("→ Parsing variation_type: $variation");

                if (!$variation) continue;

                preg_match_all('/\d+/', $variation, $matches);

                if (!empty($matches[0])) {
                    $branchOptions = array_merge($branchOptions, $matches[0]);
                    Log::info("→ Extracted numeric options:", $matches[0]);
                }
            }

            $branchOptions = array_unique($branchOptions);

            Log::info("→ FINAL Allowed Options:", $branchOptions);

            if (!empty($product->choice_options)) {

                $choiceOptions = json_decode($product->choice_options, true);

                Log::info("→ Product Original Choice Options:", $choiceOptions);

                foreach ($choiceOptions as $key => $choice) {

                    Log::info("→ Filtering Choice: {$choice['name']}");

                    $choiceOptions[$key]['options'] = array_values(array_filter(
                        $choice['options'],
                        function ($opt) use ($branchOptions) {

                            preg_match('/\d+/', $opt, $num);
                            if (!$num) return false;

                            $allow = in_array($num[0], $branchOptions);

                            Log::info("   - Checking Option '$opt' (numeric={$num[0]}) : " . ($allow ? '✔ KEEP' : '✘ REMOVE'));
                ...
                // FINAL FIX - UI will now show only filtered options
                $product->setAttribute('choice_options', json_encode($choiceOptions));
$product->filtered_choice_options = json_encode($choiceOptions);

                Log::info("→ Final Choice Options saved:", $choiceOptions);
            }
    ...
    public function getQuickView(Request $request): JsonResponse
    {
        ...
        $extraCharges['exchange'] = $extraCharges['exchange'] ?? 0; // Default to 0 if not found
        $extraCharges['installation'] = $extraCharges['installation'] ?? 0; // Default to 0 if not found

        $product->extraCharges = $extraCharges;

        // Return the product view and the extra charges data in the JSON response
        return response()->json([
            'success' => 1,
            'view' => view(POS::QUICK_VIEW[VIEW], compact('product'))->render(), // Render view with the product
