...
class WebController extends Controller
{
    ...
    public function maintenance_mode(): View
    {
    ...
    //     Log::info('Active Branches Except ID 1', [
    //         'branches' => $branches->pluck('id')
    //     ]);
    //     $aCartProduct = Cart::where(['customer_id' => auth('customer')->id()])->pluck('product_id');
    ...
    //     $shippingCountryCodes = State::distinct()->pluck('country')->toArray(); // countries with states
    //     $blacklistedCodes = DeliveryCountryCode::pluck('country_code')->toArray();

    //     $shippingCountries = array_values(array_filter($allCountries, function ($country) use ($shippingCountryCodes, $blacklistedCodes) {
    //         return in_array($country['code'], $shippingCountryCodes) && !in_array($country['code'], $blacklistedCodes);
    //     }));
    ...
    //     // --- Prepare country names & codes for view ---
    //     $shippingCountryNames = [];
    //     $shippingCountryCodesArr = [];
    //     foreach ($shippingCountries as $country) {
    //         $shippingCountryNames[] = $country['name'];
    //         $shippingCountryCodesArr[] = $country['code'];
    //     }

    //     $billingCountryNames = [];
    //     $billingCountryCodesArr = [];
    //     foreach ($billingCountries as $country) {
    //         $billingCountryNames[] = $country['name'];
    //         $billingCountryCodesArr[] = $country['code'];
    //     }
    ...
    //         'shippingCountriesCode' => $shippingCountryCodesArr,
    //         'billingCountriesName' => $billingCountryNames,
    //         'billingCountriesCode' => $billingCountryCodesArr,
    //         'billing_input_by_customer' => $billingInputByCustomer,
    //         'default_location' => $defaultLocation,
    //         'shipping_addresses' => $shippingAddresses,
    //         'billing_addresses' => $shippingAddresses,
    ...
    public function checkout_details(Request $request)
    {
        ...
        $shippingCountries = array_values(array_filter($allCountries, function ($country) use ($shippingCountryCodes) {
        ...
        // DeliveryState entries are allowed shipping states
        $allowedStateIds = DeliveryState::pluck('state_id')->toArray();
        $stateRestrictStatus = (int)getWebConfig(name: 'delivery_state_restriction');

        // Shipping States (restricted)
        $shippingStateQuery = State::query();

        // Apply country restriction (already working)
        if ($countryRestrictStatus) {
            $shippingStateQuery->whereIn('country', $shippingCountryCodes);
        }

        if ($stateRestrictStatus === 1) {
            if (empty($allowedStateIds)) {
                $shippingStateQuery->whereRaw('1 = 0');
            } else {
                $shippingStateQuery->whereIn('id', $allowedStateIds);
            }
        }

        $shippingStates = $shippingStateQuery->get();

        // Billing States (NO restriction)
        $billingStates = State::all();
    ...
    public function checkout_payment(Request $request): View|RedirectResponse
    {
        ...
        $cartPriceSummary = CartManager::getCartPriceSummary(type: 'checked');
        $couponDiscount = $cartPriceSummary['couponDiscount'];
        $amount = $cartPriceSummary['totalAmount'];
    ...
    public function getCashOnDeliveryCheckoutComplete(Request $request): View|RedirectResponse|JsonResponse
    {
        ...
        if ($physicalProductExist) {
            ...
            foreach ($cartGroupIds as $groupId) {
                ...
                \App\Services\CrmService::createOrUpdateDeal($order, 'Pending Order');
            }
    ...
    {
        $cartPriceSummary = CartManager::getCartPriceSummary(type: 'checked');
        $paymentAmount = $cartPriceSummary['totalAmount'];
    ...
    public function contact_store(Request $request)
    {
        ...
        InboxMessage::create([
            ...
            'body'          => 'New contact form submitted',
            'sender_name'   => $request['name'],
            'sender_email'  => $request['email'],
            'sender_phone'  => $request['mobile_number'],
            'pipeline'      => 'form',
    ...
    public function checkValidationForCheckoutPages(Request $request): array
    {
        ...
        $message = [];
        $deliveryType = data_get($request, 'delivery_type', session('delivery_type'));
        $shouldCheckBranchStock = $deliveryType === 'pickup';
    ...
    public function fFetchAreas(Request $request): JsonResponse
    {
        $latitude = $request->input('latitude');
        $longitude = $request->input('longitude');

        $areas = ShippingMethodArea::where('status', true)->get(); // Fetch only active areas
        // dd($areas);
        foreach ($areas as $area) {
            ...
            if ($polygon != null && $this->isPointInPolygon($latitude, $longitude, $polygon)) {
                ...
                $branchData = $branches->map(function ($branch) use ($aCartProduct, $aSystemBranch) {
                    $uniqueBranches = ManageBranchProductStock::with('branch')
                        ->whereIn('product_id', $aCartProduct)
                        ->where('branch_id', $branch->id)
                        ->get()
                        ->unique('branch_id') // Filters by unique branch_id
                        ->pluck('branch'); // Extracts the branch models
                    if (!empty($branchData)) {
                        return [
                            'id' => $branch->id,
                        ...
                        return [
                            'id' => $aSystemBranch[0]['id'],
                            'name' => $aSystemBranch[0]['branch_name'],
                            'address' => $aSystemBranch[0]['branch_address']
                        ];
                    }
                });
                return response()->json([
