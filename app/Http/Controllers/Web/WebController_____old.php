...
class WebController extends Controller
{
    ...
    public function checkout_details(Request $request)
    {
        ...
        // new changes by gannu
        $allCountries = COUNTRIES;

        // blacklist codes
        $blacklistedCodes = DeliveryCountryCode::pluck('country_code')->toArray();

        // shipping countries → remove blacklist
        $shippingCountries = array_values(array_filter($allCountries, function ($country) use ($blacklistedCodes) {
            return !in_array($country['code'], $blacklistedCodes);
        }));

        // billing countries → show all
    ...
    public function checkValidationForCheckoutPages(Request $request): array
    {
        ...
        if ($verifyStatus['status'] == 0) {
            ...
            // Create placeholder order for checkout error
            $this->createPlaceholderOrderForCheckoutError($request, 'Minimum order amount not met');
        }
        ...
        if (count($cartItemGroupIDsAll) <= 0) {
            ...
            // Create placeholder order for checkout error
            $this->createPlaceholderOrderForCheckoutError($request, 'Empty cart');
            ...
            // Create placeholder order for checkout error
            $this->createPlaceholderOrderForCheckoutError($request, 'No items checked for shipping');
        }
        ...
        foreach ($cartItemGroupIDs as $groupId) {
            ...
            if (!$productStockCheck) {
                ...
                // Create placeholder order for checkout error
                $this->createPlaceholderOrderForCheckoutError($request, 'Product out of stock');
            }
                    ...
                    if ($isPhysicalProductExist && $shippingType == 'order_wise') {
                        ...
                        if ($shippingMethod == 'inhouse_shipping') {
                            ...
                            if ($sellerShippingCount <= 0 && isset($cart->seller->shop)) {
                                ...
                                // Create placeholder order for checkout error
                                $this->createPlaceholderOrderForCheckoutError($request, 'Shipping not available for admin');
                            }
                        } else {
                            if ($cart->seller_is == 'admin') {
                                $sellerShippingCount = ShippingMethod::where(['status' => 1])->where(['creator_type' => 'admin'])->count();
                                ...
                                if ($sellerShippingCount <= 0 && isset($cart->seller->shop)) {
                                    ...
                                    // Create placeholder order for checkout error
                                    $this->createPlaceholderOrderForCheckoutError($request, 'Shipping not available for seller');
                                }
                            }
                        }
                        ...
                        if ($sellerShippingCount > 0 && $shippingMethod == 'inhouse_shipping' && $inhouseShippingMsgCount < 1) {
                            ...
                            if (!isset($cartShipping)) {
                                ...
                                // Create placeholder order for checkout error
                                $this->createPlaceholderOrderForCheckoutError($request, 'Shipping method not selected');
                            }
                            ...
                            if (!isset($cartShipping)) {
                                ...
                                // Create placeholder order for checkout error
                                $this->createPlaceholderOrderForCheckoutError($request, 'Shipping method not selected for ' . $shopIdentity);
                            }
                        }
                    }
                }
            ...
            if ($unavailableVendorsStatus) {
                ...
                // Create placeholder order for checkout error
                $this->createPlaceholderOrderForCheckoutError($request, 'Unavailable vendors');
            }
    ...
    private function createPlaceholderOrderForCheckoutError(Request $request, string $errorReason)
    {
        $user = Helpers::getCustomerInformation($request);
        $customerId = $user == 'offline' ? (session('guest_id') ?? null) : $user->id;
        $orderData = [
            ...
            'cart_group_id' => null,
            'order_amount' => CartManager::cart_grand_total(type: 'checked') ?? 0,
        ];

        $order = Order::create($orderData);

        \App\Services\CrmService::createOrUpdateDeal($order, 'Checkout Error');
    }
