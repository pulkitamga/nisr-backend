...
class HomeControllerOld extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        $robotsMetaContentData = $this->robotsMetaContentRepo->getFirstWhere(params: ['page_name' => 'home']);
        ...
        $sections = HomePageSection::with('translations')->get();
        $sectionData = [];
        $locale = getDefaultLanguage();

        foreach ($sections as $section) {
            Log::info("🔍 Section Type: {$section->type} (ID: {$section->id})");
            Log::info("🌐 Current Locale: {$locale}");

            if ($section->translations->isEmpty()) {
                Log::warning("⚠️ No translations found for section: {$section->type}");
            } else {
                foreach ($section->translations as $translation) {
                    Log::info("📄 Translation", [
                        ...
                        'translationable_id' => $translation->translationable_id ?? null,
                        'homepage_section_id' => $translation->homepage_section_id ?? null,
                    ]);
                }
            }

            $data = !empty($section->value) ? json_decode($section->value, true) : [];


            if ($section->type === 'client_review' && isset($data['clients']) && is_array($data['clients'])) {
                foreach ($data['clients'] as $i => &$client) {
                    foreach ($section->translations as $translation) {
                        if (
                            $translation->locale === $locale &&
                            isset($translation->item_index) &&
                            (int)$translation->item_index === $i &&
                            isset($client[$translation->key])
                        ) {
                            $client[$translation->key] = $translation->value;
                        }
                    }
                }
            }
            ...
            if ($section->type === 'why_choose_us') {
                ...
                // 🔁 Translate section title and subtitle (item_index = -1)
                if (isset($data['section'])) {
                    foreach ($section->translations as $translation) {
                        if (
                            $translation->locale === $locale &&
                            (int)$translation->item_index === -1 &&
                            isset($data['section'][$translation->key])
                        ) {
                ...
                // 🔁 Translate cards (item_index = 0, 1, 2, ...)
                if (isset($data['section']['cards'])) {
                    foreach ($data['section']['cards'] as $i => &$card) {
                        foreach ($section->translations as $translation) {
                            if (
                                $translation->locale === $locale &&
                                isset($translation->item_index) &&
                                (int)$translation->item_index === $i &&
                                isset($card[$translation->key])
                            ) {
                                $card[$translation->key] = $translation->value;
                            }
                            ...
                            if (
                                ...
                                isset($translation->item_index) &&
                                (int)$translation->item_index === $i &&
                                isset($card['icon'][$translation->key])
                            ) {
                                $card['icon'][$translation->key] = $translation->value;
                            }
                        }
                    }
                }
            }
            if ($section->type === 'why_join_us') {
                $translatedFields = ['title', 'description', 'image_alt'];

                foreach ($section->translations as $translation) {
                    if ($translation->locale === $locale) {
                        if (
                            in_array($translation->key, $translatedFields) &&
                            is_numeric($translation->item_index) &&
                            (int)$translation->item_index >= 0
                        ) {
                            $values = explode('|||', $translation->value ?? '');
                            foreach ($values as $i => $val) {
                                if (isset($data['section']['cards'][$i])) {
                                    $data['section']['cards'][$i][$translation->key] = $val;
                                }
                            }
                        }

                        // 🧩 Section-level title/subtitle
                        elseif ((int)$translation->item_index === -1) {
                            if (in_array($translation->key, ['title', 'subtitle', 'title_highlight'])) {
                                $data['section'][$translation->key] = $translation->value;
                            }
                        }
                    }
            ...
            foreach ($section->translations as $translation) {
                if ($translation->locale === $locale && (is_null($translation->item_index) || $translation->item_index === 0)) {
                    $globalTranslations[$translation->key] = $translation->value;
                }
            }

            if (is_array($data) && isset($data[0]) && is_array($data[0])) {
                foreach ($data as $i => &$item) {
                    $hasItemIndexTranslation = false;
                    ...
                    foreach ($section->translations as $translation) {
                        if (
                            ...
                            isset($translation->item_index) &&
                            (int)$translation->item_index === $i &&
                            isset($item[$translation->key])
                        ) {
                            $item[$translation->key] = $translation->value;
                            $hasItemIndexTranslation = true;
                        }
                    }

                    if (!$hasItemIndexTranslation) {
                        foreach ($section->translations as $translation) {
                            if (
                                $translation->locale === $locale &&
                                is_null($translation->item_index) &&
                                $translation->translationable_id == $section->id &&
                                isset($item[$translation->key])
                            ) {
                                $item[$translation->key] = $translation->value;
                            }
                        ...
                        foreach ($globalTranslations as $key => $value) {
                ...
                // Global translate into root level
                foreach ($globalTranslations as $key => $value) {
                    $originalValue = $this->getOriginalEnglishValue($key, $section->value);
                    ...
                    if (
                        ...
                        Log::info("🧩 Global translation injected at root", [
                            'key' => $key,
                            'value' => $value,
                        ]);
                    }
                ...
                // Special check for nested 'content' keys
                if (isset($data['content']) && is_array($data['content'])) {
                    foreach ($globalTranslations as $key => $value) {
                        $originalValue = $this->getOriginalEnglishValue($key, $section->value, 'content');

                        if (
                            !isset($data['content'][$key]) ||
                            empty($data['content'][$key]) ||
                            $data['content'][$key] === $originalValue
                        ) {
                            $data['content'][$key] = $value;

                            Log::info("🧩 Injected into content", [
                                'key' => $key,
                                'value' => $value,
                            ]);
                        }
    ...
    function getOriginalEnglishValue($key, $json, $sub = null)
    {
        ...
        if ($sub && isset($original[$sub]) && is_array($original[$sub])) {
            return $original[$sub][$key] ?? null;
        }

        return $original[$key] ?? null;
    }

    public function store(): View
    {

        $robotsMetaContentData = $this->robotsMetaContentRepo->getFirstWhere(params: ['page_name' => 'store']);
        if (!$robotsMetaContentData) {
            $robotsMetaContentData = $this->robotsMetaContentRepo->getFirstWhere(params: ['page_name' => 'default']);
        }
        $brands = $this->cachePriorityWiseBrandList();
        ...
        $featuredProductsList = ProductManager::getPriorityWiseFeaturedProductsQuery(query: $this->product->active()->where('product_type', '!=', 'services')->with(['clearanceSale' => function ($query) {
            return $query->active();
        }]), dataLimit: 12);
        $newArrivalProducts = ProductManager::getPriorityWiseNewArrivalProductsQuery(query: $this->product->active()->where('product_type', '!=', 'services')->with(['clearanceSale' => function ($query) {
            return $query->active();
        }]), dataLimit: 8);
        $wholesaleProducts = null;
        if (auth('customer')->check() && auth('customer')->user()?->user_type == 1) {
            $wholesaleProducts = WholeSaleProducts::where('status', 1)
                ->with(['product', 'price_list'])
                ->get();
        }
        $dealOfTheDay = DealOfTheDay::with(['product' => function ($query) {
            return $query->active()->with(['clearanceSale' => function ($query) {
        ...
        return view('default.web-views.pages.store', compact(
            'flashDeal',
            'featuredProductsList',
            'topRatedProducts',
            'bestSellProduct',
