      ...
      public function place_order(Request $request): JsonResponse
    {
        ...
        foreach ($cartGroupIds as $groupId) {
            $data = [
                ...
                'transfer_from_branch_override' => $deliveryType === 'pickup'
                    ? $pickupBranchId
            ...
            if ($deliveryType === 'pickup') {
                $order->transfer_from_branch = $pickupBranchId;
                $order->pickup_from_branch = $pickupBranchId;
                $order->shipping_address = null;
                $order->shipping_address_data = null;
                $order->shipping_type = 'pickup';
            } else {
                $order->pickup_from_branch = 0;
...
$order->order_note = ($request['order_note'] != null) ? $request['order_note'] : $order['order_note'];
    ...
    public function placeOrderByOfflinePayment(Request $request): JsonResponse
    {
        ...
        if ($physicalProductExist && $deliveryType !== 'pickup') {
            $zipRestrictStatus = getWebConfig(name: 'delivery_zip_code_area_restriction');
            $countryRestrictStatus = getWebConfig(name: 'delivery_country_restriction');
    ...
    public function placeOrderByWallet(Request $request): JsonResponse
    {
        ...
        if (count($cart_group_ids) === 0 || $carts->isEmpty()) {
            return response()->json(['message' => 'No checked cart items found'], 403);
        }
