...
class POSController extends Controller
{
    ...
    public function getProductStockCalculate($cartItem, $product, int $sellerId, ?int $branchId, int $orderId): array
    {
        if ($product['product_type'] !== 'physical') {
            return ['status' => true, 'message' => '', 'branch_id' => null];
        }
        ...
        $stockMutation = $this->inventoryMutationService->decreaseForPosLine(
            ...
            sellerId: $sellerId,
            referenceId: $orderId,
            context: 'Seller Mobile POS'
        );

        return [
            'status' => (bool)($stockMutation['status'] ?? false),
            'message' => $stockMutation['message'] ?? '',
            'branch_id' => isset($stockMutation['branchId']) ? (int)$stockMutation['branchId'] : null,
        ];
    }
    ...
    public function place_order(Request $request): JsonResponse
    {
        ...
        foreach ($carts as $cartItem) {
            if (is_array($cartItem)) {
                ...
                if ($product) {
                    ...
                    $stockMutation = $this->getProductStockCalculate(
                        cartItem: $cartItem,
                        product: $product,
                    ...
                    if (!($stockMutation['status'] ?? false)) {
                        return response()->json([
                            'message' => $stockMutation['message'] ?? 'Stock not available for selected product/variation.'
                        ], 422);
                    }

                    if (isset($stockMutation['branch_id']) && (int)$stockMutation['branch_id'] > 0) {
                        $resolvedBranchId = (int)$stockMutation['branch_id'];
                    }
        ...
        $orderData = [
            ...
            'transfer_from_branch' => (int)($resolvedBranchId ?? 1),
