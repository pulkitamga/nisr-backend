...
class CustomerAuthController extends Controller
{
    ...
    // Extract values from request & session
    $requestValue = $request['default_recaptcha_id_customer_login'] ?? null;
    $sessionValue = Session('default_recaptcha_id_customer_login') ?? null;
        ...
        if (auth('customer')->attempt([$loginMedia => $user[$loginMedia], 'password' => $request['password']], $remember)) {
            ...
            if (auth('customer')->user()->user_type == 1) {
                ...
                if (!$exists) {
                    ...
                    return redirect()->route('wholesaler.business.add');
                }
            }
