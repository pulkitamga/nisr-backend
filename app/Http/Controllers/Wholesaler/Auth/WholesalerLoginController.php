...
class WholesalerLoginController extends Controller
{
    ...
    public function loginByEmailOrPhone(Request $request): JsonResponse|RedirectResponse
    {
        ...
        if (isset($recaptcha) && $recaptcha['status'] == 0 && $firebaseOTPVerification && !$firebaseOTPVerification['status']) {
            ...
            $requestValue = $request['default_captcha_code'] ?? null;
            $sessionValue = Session('default_captcha_code') ?? null;

            // 🔍 Log for debugging
            Log::info('Captcha Debug:', [
                'request_value' => $requestValue,
                'session_value' => $sessionValue,
                'match' => strtolower($requestValue) == strtolower($sessionValue),
            ]);

            if (strtolower($requestValue) != strtolower($sessionValue)) {
                if ($request->ajax()) {
                    return response()->json([
                        'status' => 'error',
        ...
        if (auth('customer')->attempt([$loginMedia => $user[$loginMedia], 'password' => $request['password']], $remember)) {
            ...
            // Default response (same jaise pehle tha)
            $response = [
                'status'       => 'success',
            ...
            // Wholesaler hai aur business nahi bana → force redirect to business add
            if (auth('customer')->user()->user_type == 1) {
                $exists = WholeSalerBusiness::where('wholesaler_id', auth('customer')->user()->id)->exists();

                if (!$exists) {
                    $response = [
                        'status'       => 'success',
                        'message'      => translate('please_complete_your_business_profile'),
                        'redirect_url' => route('wholesaler.business.add'),
                        'reload'       => false,
                    ];
                }
            }

            // Agar AJAX request hai → JSON return kar do (bilkul same format)
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json($response);
            }

            // Normal form submit → redirect with toastr
    ...
    public function resendOTPCode(Request $request): JsonResponse|RedirectResponse
    {
        ...
        $newTokenGenerate = \App\Support\OtpManager::numericToken(4);
        if ($timeDifferance == 0) {
            if ($token) {
