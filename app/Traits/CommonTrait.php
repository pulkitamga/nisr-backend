...
trait CommonTrait
{
    ...
    {
        $input_country = strtolower($input_country);

        $allowedCountries = $this->get_delivery_country_array();

        $allowedNames = array_map(fn($c) => strtolower($c['name']), $allowedCountries);
        $allowedCodes = array_map(fn($c) => strtolower($c['code']), $allowedCountries);

        return in_array($input_country, $allowedNames) || in_array($input_country, $allowedCodes);
    }
