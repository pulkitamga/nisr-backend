...
class WholesaleOrderRepository implements WholesaleOrderRepositoryInterface
{

    public function __construct(
        private readonly WholesaleConfirmOrder        $order,
        private readonly WholesalePurchaseOrder       $purchaseOrder,
        private readonly WholesaleQuotation           $quotation,
    ...
    public function getTopWholesalerList(array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->order // instance of WholesaleConfirmOrder model
            ->with($relations)
            ->where($filters)
            ->select('wholesaler_id', DB::raw('COUNT(wholesaler_id) as count'))
            ->whereHas('wholeseller', function ($q) {
                $q->where('id', '!=', 0); // ensure valid user
            })
