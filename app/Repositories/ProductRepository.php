...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function getWebFirstWhereActive(array $params, array $relations = [], array $withCount = []): ?Model
    {
        return $this->product->active()
            ...
            ->when(isset($relations['translations']), function ($query) use ($relations) {
                return $query->with($relations['translations']);
            })
    ...
    public function getListWhere(array $orderBy = [], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->product->with($relations)->when(isset($filters['added_by']) && $this->isAddedByInHouse(addedBy: $filters['added_by']), function ($query) {
            ...
            })->when(isset($filters['branch_id']) && !empty($filters['branch_id']), function ($query) use ($filters) {
                return $query->where('branch_id', $filters['branch_id']);
            })
            ->when(isset($filters['product_type']) && !empty($filters['product_type']), function ($query) use ($filters) {
    ...
    public function getStockLimitProductsWhere(array $orderBy = [], string $searchValue = null, array $filters = [], array $withCount = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->product->with($relations)
            ->withCount($withCount)
            ->when($this->isAddedByInHouse(addedBy: $filters['added_by']), function ($query) {
