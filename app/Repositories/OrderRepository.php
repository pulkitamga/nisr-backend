...
class OrderRepository implements OrderRepositoryInterface
{
    ...
    public function updateStockOnOrderStatusChange(string|int $orderId, string $status): bool
    {
        ...
        if ($normalizedStatus == 'delivered') {
            $response = $inventoryMutationService->decreaseForOrderById(orderId: $orderId);
        } elseif (in_array($normalizedStatus, ['returned', 'failed', 'canceled'], true)) {
            $response = $inventoryMutationService->increaseForReturnOrCancelById(orderId: $orderId, status: $normalizedStatus);
        } else {
            $response = $inventoryMutationService->applyOrderStatusChangeById(orderId: $orderId, status: $normalizedStatus);
        }
