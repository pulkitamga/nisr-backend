    ...
    public static function getCategoriesWithCountingAndPriorityWiseSorting($dataLimit = null)
    {
        ...
        $categories = Cache::remember($cacheKey, CACHE_FOR_3_HOURS, function () {
            return Category::with(['product' => function ($query) {
                return $query->active()->where('product_type', '!=', 'services')->withCount(['orderDetails'])->with(['clearanceSale' => function ($query) {
                    return $query->active();
                }]);
            }])->withCount(['product' => function ($query) {
                $query->active()->where('product_type', '!=', 'services')->when(request('offer_type') == 'clearance_sale', function ($query) {
