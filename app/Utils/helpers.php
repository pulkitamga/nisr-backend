...
class helpers
{
    ...
    //     } elseif (
    //         isset($request['payment_request_from'])
    //         && in_array($request['payment_request_from'], ['app'])
    //     ) {
    //         // TRUST AUTH TOKEN FIRST
    //         if (auth('customer')->check()) {
    ...
    public static function isTranslated($text, $locale): bool
    {
        $messages = @include(base_path("resources/lang/{$locale}/messages.php"));
        ...
        public static function set_data_format($data)
{
    ...
    public static function tax_calculation($product, $price, $tax, $tax_type)
    {
        ...
        $taxModel = is_array($product) ? ($product['tax_model'] ?? 'exclude') : ($product->tax_model ?? 'exclude');
        if ($taxModel == 'include') {
