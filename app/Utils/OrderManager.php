...
class OrderManager
{
    ...
    // {
    //     $order = Order::with(['deliveryMan', 'pickupBranch', 'orderStatusHistory' => function ($query) {
    //         return $query->latest();
    ...
    public static function track_order($order_id)
    {
        // Load order with deliveryMan, latest orderStatusHistory, and the order details
        $order = Order::with([
            'deliveryMan',
            'orderStatusHistory' => function ($query) {
                return $query->latest();
            },
            'details' // <-- Add this to get tax and other details
        ])->where('id', $order_id)->first();

        // Calculate total tax from order details
        $order->tax_total = $order->details->sum('tax');
    ...
    {
        $minimumOrderId = 100001;
        $maxOrderId = (int)(Order::max('id') ?? 0);
        $maxOrderIdFromDetails = (int)(OrderDetail::max('order_id') ?? 0);

        $nextOrderId = max($minimumOrderId, $maxOrderId + 1, $maxOrderIdFromDetails + 1);

        while (
            Order::where('id', $nextOrderId)->exists()
            || OrderDetail::where('order_id', $nextOrderId)->exists()
        ) {
            $nextOrderId++;
        }
    ...
    public static function order_summary($order)
    {
        ...
        foreach ($order->details as $detail) {
            $linePrice = (float)$detail->price * (float)$detail->qty;
            $lineDiscount = (float)$detail->discount;
            $sub_total += $linePrice;
            $total_discount_on_product += $lineDiscount;

            $taxModel = $detail['tax_model'] ?? 'exclude';
            $lineAmount = $taxModel == 'include'
                ? max(0, ($linePrice + (float)$detail->tax) - $lineDiscount)
                : max(0, $linePrice - $lineDiscount);
            $lineAmountTotal += $lineAmount;

            $lineItems[] = [
                'amount' => $lineAmount,
                'tax_rate' => self::getOrderDetailTaxRate($detail),
                'tax_model' => $taxModel,
            ];
        }
        ...
        $netBeforeTaxAfterCoupon = 0;
        $total_tax = 0;
        $subtotalWithTaxAfterCoupon = 0;
        ...
        foreach ($lineItems as $index => $lineItem) {
            ...
            $taxBreakdown = CartManager::getTaxBreakdownFromAmount(
                amount: $lineAmountAfterCoupon,
                taxRate: $lineItem['tax_rate'],
                taxModel: $lineItem['tax_model']
            );
            $netBeforeTaxAfterCoupon += $taxBreakdown['net'];
            $total_tax += $taxBreakdown['vat'];
            $subtotalWithTaxAfterCoupon += $taxBreakdown['gross'];
        }
        ...
        return [
            ...
            'net_before_tax_after_coupon' => $netBeforeTaxAfterCoupon,
            'subtotal_with_tax_after_coupon' => $subtotalWithTaxAfterCoupon,
            'coupon_discount_on_product' => $couponDiscountOnProduct,
            'coupon_discount_on_shipping' => $couponDiscountOnShipping,
        ];
    }

    private static function getOrderCouponType($order): ?string
    {
        if (isset($order->coupon) && isset($order->coupon->coupon_type)) {
            return $order->coupon->coupon_type;
        }

        $couponCode = $order['coupon_code'] ?? null;
        if (in_array($couponCode, [null, '', 0, '0'], true)) {
            return null;
        }

        return Coupon::where('code', $couponCode)->value('coupon_type');
    }

    private static function getOrderDetailTaxRate($detail): float
    {
        $productDetails = $detail['product_details'] ?? null;
        if (is_string($productDetails)) {
            $decoded = json_decode($productDetails, true);
            $productDetails = is_array($decoded) ? $decoded : [];
        } elseif (is_object($productDetails)) {
            $productDetails = (array)$productDetails;
        } elseif (!is_array($productDetails)) {
            $productDetails = [];
        }

        $taxRate = (float)($productDetails['tax'] ?? 0);
        if ($taxRate > 0) {
            return $taxRate;
        }

        if (isset($detail->productAllStatus) && isset($detail->productAllStatus->tax)) {
            $taxRate = (float)$detail->productAllStatus->tax;
            if ($taxRate > 0) {
                return $taxRate;
            }
        }
        ...
        $linePrice = (float)$detail->price * (float)$detail->qty;
        $lineTax = (float)$detail->tax;
        if ($linePrice > 0 && $lineTax > 0) {
            return ($lineTax / $linePrice) * 100;
        }
    ...
    public static function stock_update_on_order_status_change($order, $status)
    {
        ...
        if ($normalizedStatus == 'delivered') {
            $response = $inventoryMutationService->decreaseForOrder(order: $order);
        } elseif (in_array($normalizedStatus, ['returned', 'failed', 'canceled'], true)) {
            $response = $inventoryMutationService->increaseForReturnOrCancel(order: $order, status: $normalizedStatus);
        } else {
            $response = $inventoryMutationService->applyOrderStatusChange(order: $order, status: $normalizedStatus);
        }
    ...
    public static function wallet_manage_on_order_status_change($order, $received_by)
    {
        ...
        $order_amount = $order_summary['net_before_tax_after_coupon'];
    ...
    public static function coupon_process($data, $coupon)
    {
        ...
        foreach ($carts as $cart) {
            if (($coupon->seller_id == NULL && $cart->seller_is == 'admin') || $coupon->seller_id == '0' || ($coupon->seller_id == $cart->seller_id && $cart->seller_is == 'seller')) {
                $total_amount += (($cart['price'] - $cart['discount']) * $cart['quantity']);
            }
        }
    ...
    public static function getApiPayableAmount($request): float
    {
        ...
        foreach ($cartGroupIds as $cartGroupId) {
            $groupDiscount = 0;
            $groupCouponType = null;

            if ($coupon) {
                $couponProcess = self::coupon_process(
                    data: [
                ...
                $groupDiscount = (float)($couponProcess['discount'] ?? 0);
                $groupCouponType = $couponProcess['coupon_type'] ?? null;
            }

            $summary = CartManager::getCartPriceSummary(
                cartGroupId: $cartGroupId,
                type: 'checked',
                couponDiscount: $groupDiscount,
                couponType: $groupCouponType,
                request: $request
            );
    ...
    public static function generate_order($data)
    {
        ...
        if ($free_delivery['status'] && $free_delivery['shipping_cost_saved'] > 0 && $couponProcess['coupon_type'] != 'free_delivery') {
            $is_shipping_free = 1;
            $freeShippingDiscount = CartManager::get_shipping_cost(groupId: $data['cart_group_id'], type: 'checked', request: $req);
        ...
        foreach (CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked') as $cartSingleItem) {
            // $installation = $cartSingleItem['installtion_charges'] * $cartSingleItem['quantity'];
            $installation = $cartSingleItem['installtion_charges'];
            $exchange = $cartSingleItem['exchange_charges'] * $cartSingleItem['exchange_qty'];

            $totalInstallationCharge += $installation;
            $totalExchangeCharge += $exchange;
        }

        $orderAmountSummary = CartManager::getCartPriceSummary(
            cartGroupId: $cart_group_id,
            type: 'checked',
            couponDiscount: (float)$discount,
            couponType: $couponProcess['coupon_type'] ?? null,
            request: $req
        );

        $deliveryType = data_get($req, 'delivery_type', session()->has('delivery_type') ? session('delivery_type') : 'delivery');
        $transferFromBranchOverride = isset($data['transfer_from_branch_override']) ? (int)$data['transfer_from_branch_override'] : 0;

        if ($transferFromBranchOverride > 0) {
            $transfer_from_branch = $transferFromBranchOverride;
        } else {
            $transfer_from_branch = (int)data_get($req, 'transfer_from_branch', 0);
            if ($transfer_from_branch <= 0) {
                $transfer_from_branch = (int)data_get($req, 'nearest_branch', 0);
            }
            if ($transfer_from_branch <= 0) {
                $transfer_from_branch = (int)(session()->has('nearest_branch') ? session('nearest_branch') : 0);
            }
            if ($transfer_from_branch <= 0) {
        ...
        if ($deliveryType == 'pickup') {
            $pickupBranchId = (int)data_get($req, 'pickup_branch_id', session('pickup_branch_id') ?? $transfer_from_branch);
            if ($pickupBranchId > 0) {
                $transfer_from_branch = $pickupBranchId;
            }
        }

        $pickupFromBranch = $deliveryType == 'pickup' ? $transfer_from_branch : 0;
        $shippingAddressIdForOrder = $deliveryType == 'pickup' ? null : $address_id;
        $shippingAddressDataForOrder = $deliveryType == 'pickup' ? null : ShippingAddress::find($address_id);
        ...
        $or = [
            ...
            'installation_charge' => $totalInstallationCharge,
            'exchange_charge' => $totalExchangeCharge,
            ...
            'order_amount' => max(0, $orderAmountSummary['totalAmount']),
            'admin_commission' => $admin_commission,
            'shipping_address' => $shippingAddressIdForOrder,
            'shipping_address_data' => $shippingAddressDataForOrder,
            ...
            'shipping_cost' => CartManager::get_shipping_cost(groupId: $data['cart_group_id'], type: 'checked', request: $req),
            ...
            'shipping_type' => $deliveryType == 'pickup' ? 'pickup' : $shipping_type,
        ...
        foreach (CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked') as $cartSingleItem) {
            ...
            $orderDetails = [
                ...
                'installtion_charges' => $cartSingleItem['installtion_charges'],
                'exchange_qty' => $cartSingleItem['exchange_qty'],
                'exchange_charges' => $cartSingleItem['exchange_charges'],
    ...
    {
        $requestContext = data_get($order_data, 'data.request');
        $orderAmountSummary = CartManager::getCartPriceSummary(
            cartGroupId: $order_data['cart_group_id'],
            type: 'checked',
            couponDiscount: (float)$order_data['discount'],
            couponType: ($order_data['coupon_type'] ?? null),
        ...
        $or = [
            ...
            'shipping_cost' => CartManager::get_shipping_cost(groupId: $order_data['data']['cart_group_id'], type: 'checked', request: $requestContext),
