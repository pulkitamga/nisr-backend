...
class ProductManager
{
    ...
    public static function get_seller_products($seller_id, $request)
    {
        ...
        $productModel = $request->get('product_model', '');
      $matchModels = $request->get('match_models', []);

// Normalize to array
if (!is_array($matchModels)) {
    // If string "null" or empty, make it empty array
    if ($matchModels === 'null' || $matchModels === null || trim($matchModels) === '') {
        $matchModels = [];
    } else {
        // If string value (like "Civic"), wrap into array
        $matchModels = [$matchModels];
    }
}

// Remove empty/null items just in case
$matchModels = array_filter($matchModels, fn($val) => $val !== null && $val !== '');
        ...
        $products = Product::active()
            ...
            ->when($request->has('category') && $categories, function ($query) use ($categories) {
                $query->where(function ($query) use ($categories) {
                  ...
                  // Add product_model filter
            ->when(!empty($productModel), function ($query) use ($productModel) {
            $query->where('product_model', 'like', "%{$productModel}%");
            })
        
        // Add match_model filter
      ->when(is_array($matchModels) && count($matchModels) > 0, function ($query) use ($matchModels) {
    $query->where(function ($q) use ($matchModels) {
        $q->whereNotNull('match_models');
        foreach ($matchModels as $model) {
            $q->orWhereJsonContains('match_models', $model);
        }
    });
})
    ...
    public static function getPriorityWiseCategoryWiseProductsQuery($query, $dataLimit = 'all', $offset = 1, $appends = null)
    {
        ...
        if ($categoryWiseProductSortBy && ($categoryWiseProductSortBy['custom_sorting_status'] == 1)) {
            ...
            $query = $query->filter(function ($product) {
                return $product->product_type !== 'services';
            });
        ...
        if ($dataLimit != 'all') {
            return $query->where('product_type', '!=', 'services')->paginate($dataLimit, ['*'], 'page', request()->get('page', $offset));
        }

        return $query->where('product_type', '!=', 'services')->get();
    }
    ...
    public static function getProductListData($request, $productUserID = null, $productAddedBy = null): mixed
    {
        ...
        $productListData = Product::active()
            ...
            ->when($request->filled('make'), function ($query) use ($request) {
                return $query->whereJsonContains('match_makes', strtolower($request->make));
            })
            ->when($request->filled('model'), function ($query) use ($request) {
                return $query->whereJsonContains('match_models', strtolower($request->model));
            })
            ->when($request->filled('year'), function ($query) use ($request) {
                // Important: year as string if stored as string in JSON
                return $query->whereJsonContains('match_years', (string) $request->year);
            })
            ->when(in_array($request['product_type'], ['physical', 'digital']), function ($query) use ($request) {
                return $query->where(['product_type' => $request['product_type']]);
            })
            ...
            ->when($request['offer_type'] == 'discounted', function ($query) {
                $stockClearanceProductIds = StockClearanceProduct::active()->pluck('product_id')->toArray();
                return  $query->where(function ($q) use ($stockClearanceProductIds) {
                    $q->where('discount', '>', 0);
                    if (!empty($stockClearanceProductIds)) {
                        $q->orWhereIn('id', $stockClearanceProductIds);
                    }
                });
            })
