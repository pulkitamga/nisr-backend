    ...
    function getStorageImages($path, $type = null, $source = null): string
    {
        ...
        if (isset($placeholderMap[$type])) {
            ...
            if (is_array($placeholderMap[$type])) {
                ...
                $placeholderPath = theme_asset($placeholderMap[$type][$theme] ?? '');

                if ($theme == 'default') {
                    $placeholderPath = theme_asset($placeholderMap[$type][$theme] ?? '');
                }

                return (isset($path['status']) && $path['status'] == 200) ? $path['path'] : $placeholderPath;
            } else {
                return (isset($path['status']) && $path['status'] == 200)
                    ? $path['path']
                    : dynamicAsset('public/assets/' . $placeholderMap[$type]);
            }
        }
