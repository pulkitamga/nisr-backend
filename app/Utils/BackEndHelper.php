...
class BackEndHelper
{
    public static function currency_to_usd($amount)
    {
        ...
        if ($currency_model == 'multi_currency') {
            $default_currency = Currency::find(BusinessSetting::where('type', 'system_default_currency')->first()->value);
            $base = Currency::where('code', $default_currency->code)->first();
            $rate = $default_currency->exchange_rate / $base->exchange_rate;
    ...
    public static function usd_to_currency($amount)
    {
        ...
        if ($currency_model == 'multi_currency') {
            $default = session('default') ?? Currency::find(getWebConfig('system_default_currency'))->exchange_rate;
            session()->put('default', $default);

            $default_currency_code = Currency::find(getWebConfig('system_default_currency'))->code;
            $base = session($default_currency_code) ?? Currency::where('code', $default_currency_code)->first()->exchange_rate;
            session()->put($default_currency_code, $base);
