...
class ChattingService
{
    ...
    public function addChattingDataForWeb(object $request, string|int $userId, string $type, string|int $shopId = null, string|int $vendorId = null, int $adminId = null, int $deliveryManId = null): array
    {
        $resolvedShopId = $this->resolveShopIdForWebMessage(
            shopId: $shopId,
            vendorId: $vendorId,
            adminId: $adminId
        );
    ...
    private function resolveShopIdForWebMessage(string|int|null $shopId = null, string|int|null $vendorId = null, ?int $adminId = null): int|string|null
    {
        if (!is_null($shopId) && $shopId !== '') {
            return $shopId;
        }

        $isInhouseChat = ((int)$adminId === 0) || ((int)$vendorId === 0);
        if (!$isInhouseChat) {
            return $shopId;
        }

        $inhouseShopId = Shop::query()
            ->whereIn('seller_id', [0, 1])
            ->value('id');

        if (is_null($inhouseShopId)) {
            $inhouseShopId = Shop::query()->value('id');
        }

        // Fallback to 0 for projects that use virtual inhouse shop id.
        return !is_null($inhouseShopId) ? (int)$inhouseShopId : 0;
    }
}
