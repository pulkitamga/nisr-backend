...
use Illuminate\Support\Facades\Log; // Import karo agar nahi kiya hua
...
class CartServiceOld
{
    ...
    public function getVariantData(object $request, object $product, string $colorName = null): array
    {
        ...
        foreach ($cartData as $key => $cart) {
            if (
                is_int($key) && is_array($cart) && isset($cart['id'], $cart['variant']) &&
                $cart['id'] == $product['id'] && $cart['variant'] == $variation
            ) {
    ...
    public function addCartDataOnSession(object $product, int $quantity, float $price, float $discount, string $variant, array $variations, $exchangeCharge,  $installationCharge): array
    {
        ...
        if (session()->has($cartId)) {
            ...
            session()->put($cartId, [
                'items' => $keeper,
                'add_to_cart_time' => $existingSession['add_to_cart_time'] ?? Carbon::now(),
                'installation_charge' => $installationCharge,
                'exchange_charge' => $exchangeCharge,
            ]);
        } else {
    ...
    public function getCartSubtotalCalculation(object $product, array $cartItem, array $calculation): array
    {
        ...
        $installationCharge = session('cart')['installation_charge'] ?? 0;
        $exchangeCharge = session('cart')['exchange_charge'] ?? 0;

        // Debugging for charges
        \Log::info("Installation Charge from Session: " . $installationCharge);
        \Log::info("Exchange Charge from Session: " . $exchangeCharge);
