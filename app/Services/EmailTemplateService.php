...
class EmailTemplateService
{
    ...
    public function getEmailTemplateData($userType):array
    {
        ...
        $wholesaler = EmailTemplateKey::WHOLESALER_EMAIL_LIST;
    ...
    public function getHiddenField(string $userType, string $templateName):array|null
    {
        ...
        $hiddenData =  [
            ...
            EmailTemplateKey::QUOTATION_SEND=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::PAYMENT_RECEIVED=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::PRODUCT_SEND=>array('product_information','order_information','button_content','banner_image'),
            EmailTemplateKey::PURCHASE_ORDER_RECEIVED=>array('product_information','order_information','button_content','banner_image'),
    ...
    public function getTitleData(string $userType,$templateName):string
    {
        $titleData =  [
            ...
            EmailTemplateKey::QUOTATION_SEND=>'Quotation Send',
            EmailTemplateKey::PAYMENT_RECEIVED=>'Payment Recevied',
            EmailTemplateKey::PRODUCT_SEND=>'Product Send',
            EmailTemplateKey::PURCHASE_ORDER_RECEIVED=>'Purchase Order Recevied',
        ];
    ...
    public function getBodyData(string $userType,$templateName):string
    {
        ...
        $bodyData =  [
            ...
            EmailTemplateKey::QUOTATION_SEND=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>Your A complete quoatation of your purchase order has been sent to you. You can check in my Quotation in profile section </div><div><br></div><div>Meanwhile, click here to visit the'.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::PAYMENT_RECEIVED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>Your payment for purchess wholesale product is recevied and we process your order as soon as possible</div><div><br></div><div>Meanwhile, click here to your order and track hear'.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::PURCHASE_ORDER_RECEIVED=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>Your purchase order for wholesale product is recevied and our team review your order and send you a mail when your order is approve</div><div><br></div><div>Meanwhile, click here to your order and track hear'.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
            EmailTemplateKey::PRODUCT_SEND=>'<div><b>Hi '.$userType.',</b></div><div><b><br></b></div><div>We sent your purchess order you can track in your profile my purchase order section</div><div><br></div><div>Meanwhile, click here to visit the'.getWebConfig('company_name').' shop website</div><div><font color="#0000ff"> <a href="'.url('/').'" target="_blank">'.url('/').'</a></font></div>',
