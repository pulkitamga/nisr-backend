...
class ProductService
{
    ...
    public function getAddProductData(object $request, string $addedBy): array
    {
        ...
        return [
            ...
            'unit_price' => currencyConverter(amount: (float) ($request['unit_price'] ?? 0)),
            'purchase_price' => 0,
            ...
            'thumbnail' => $request->hasFile('image') ? $this->upload(dir: 'product/thumbnail/', format: 'webp', image: $request->file('image')) : $request->existing_thumbnail,
            'thumbnail_storage_type' => $request->hasFile('image') ? $storage : null,
            'preview_file' => $previewFile,
            'preview_file_storage_type' => $request->hasFile('preview_file') ? $storage : $request->get('existing_preview_file_storage_type', null),
            'meta_title' => $request['meta_title'],
            'meta_description' => $request['meta_description'],
            'meta_image' => $request->hasFile('meta_image') ? $this->upload(dir: 'product/meta/', format: 'webp', image: $request->file('meta_image')) : $request->existing_meta_image,
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        /*
            Physical inventory shape is immutable after creation.
            Stock changes must go through stock adjustment flows.
        */
        $lockPhysicalInventoryShape = ($product['product_type'] === 'physical' && $request['product_type'] === 'physical');
        $existingChoiceOptions = $product->getRawOriginal('choice_options') ?: json_encode([]);
        $existingVariation = $product->getRawOriginal('variation') ?: json_encode([]);
        $existingColors = $product->getRawOriginal('colors') ?: json_encode([]);
        $existingAttributes = $product->getRawOriginal('attributes') ?: json_encode([]);

        $combinations = $this->getCombinations($this->getOptions(request: $request));
        $variations = $lockPhysicalInventoryShape
            ? $this->getLockedPhysicalVariationsForUpdate(
                request: $request,
                existingVariationJson: $existingVariation
            )
            : $this->getVariations(request: $request, combinations: $combinations);
        $stockCount = $lockPhysicalInventoryShape
            ? (int)($product['current_stock'] ?? 0)
            : (isset($combinations[0]) && count($combinations[0]) > 0 ? $this->getTotalQuantity(variations: $variations) : (int)$request['current_stock']);
        ...
        $dataArray = [
            ...
            'choice_options' => $request['product_type'] == 'physical'
                ...
                : json_encode([]),
            'variation' => $request['product_type'] == 'physical'
    ...
    private function getLockedPhysicalVariationsForUpdate(object $request, string $existingVariationJson): array
    {
        $existingVariations = json_decode($existingVariationJson, true) ?: [];
        ...
        foreach ($existingVariations as $variation) {
            ...
            $priceInputKey = $this->resolveVariationInputKey($request, 'price_', $type);
            $skuInputKey = $this->resolveVariationInputKey($request, 'sku_', $type);

            $priceInput = $priceInputKey ? $request->input($priceInputKey) : null;
            $skuInput = $skuInputKey ? $request->input($skuInputKey) : null;

            $existingPrice = (float)($variation['price'] ?? 0);
            $variation['price'] = $priceInput !== null && $priceInput !== ''
                ? currencyConverter(amount: abs((float)$priceInput))
                : $existingPrice;

            if ($skuInput !== null && trim((string)$skuInput) !== '') {
                $variation['sku'] = trim((string)$skuInput);
            }

            // Keep quantity immutable in product edit flow.
            $variation['qty'] = max(0, (int)($variation['qty'] ?? 0));
            $updatedVariations[] = $variation;
        }
    ...
    private function resolveVariationInputKey(object $request, string $prefix, string $type): ?string
    {
        $exactKey = $prefix . $type;
        if ($request->has($exactKey)) {
            return $exactKey;
        }

        $normalizedKey = $prefix . str_replace('.', '_', $type);
        if ($request->has($normalizedKey)) {
    ...
    public function getProductsDropdown(object $request, object $products): string
    {
        $dropdown = '<option value="' . 0 . '" disabled selected>---' . translate("Select") . '---</option>';
